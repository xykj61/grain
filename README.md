...
**Who tends this pier.** The standing writing voice is **Kyri** ([`context/KYRI.md`](context/KYRI.md), molted from Riyo `20260810`); the living coauthor is **Keaton Livermore**, and the living **beginner clone** is **`xykj61/grain`** (`autoproject96/grain` is the agent lane). Your first-day path is [`SOURCE.md`](SOURCE.md) — from nothing to a signed, sandboxed home. **Quin** keeps the fifth OS variant and the inference Q-vane ([`context/QUIN.md`](context/QUIN.md)); the standing voice **Kyri** is also the session-log profile default.
