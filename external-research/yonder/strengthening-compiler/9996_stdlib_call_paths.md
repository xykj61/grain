...
Tiger Style: *use explicitly-sized types like `u32`; avoid architecture-specific `usize`* ([`gratitude/TIGER_STYLE.md`](../../../gratitude/TIGER_STYLE.md) section Safety).
...
Lexicon [ok] requires every row **`done`** and zero **`fail`** rows.
### `std..mem.eql`
...
| `external-research/yonder/strengthening-compiler/9996_stdlib_call_paths.md` | pass record + audited surfaces | done |
| `## usize explicit audit` | per-surface locus table -- gates lexicon [ok] | done |
