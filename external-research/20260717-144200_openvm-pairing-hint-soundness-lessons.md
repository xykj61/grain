...
Secure A plus secure B does not imply a secure AoB. ZK Security's own framing of why naive folder-parallel AI audit fails on a zkVM is the same grain this tree already names for Pool, Caravan capabilities, Comlink seals, and the TUBE ladder: **the seam carries the claim**. TUBE2 publish without resolve-and-admit, TUBE3 sealed fetch without digest match, TUBE1 admit without a matching grant -- each is the same class of miss as "optimized equation without the subfield check."
