...
2. ~~**Read the resource vocabulary beside the refusal one**~~ -- **LANDED `20260821.222402`**. [`caravan/objects.rye`](../caravan/objects.rye) states the nine riscv64 object types in their own numbering, names the tier each belongs to, and names for each the Caravan module that stands for it today. The accounting found something it did not go looking for: Caravan stands for **five** of the nine, and the five are **exactly the portable tier** -- so the four it owes are exactly the mode and architecture tiers, every one of them to an address space. [`tools/caravan_objects_witness.rish`](../tools/caravan_objects_witness.rish) binds the symbols, their order, and both tier anchors against the vendored BSD-2-Clause headers at run time, GREEN on metal with all three RED paths proven. The reading: [`the portable tier Caravan already grew`](../active-designing/20260821-222402_the-portable-tier-caravan-already-grew.md).
3. **Open the generator gate** when convenient, by adding `jinja2` and `ply` to the pier's Python environment, which unlocks `types.h`, `syscalls.h`, `bootinfo.h`, `faults.h`, and the `sel4.h` umbrella in one step. Environment change, so it is Keaton's word.
4. **A root-task skeleton** freestanding for riscv64 once the generated headers stand, linking `libsel4` rather than a stub.
5. **Aurora on RISC-V under QEMU**, where Equinox 4's close waits: the parity-witness happy-zone suite running GREEN on the new target, which is the proof that ends the arc.
