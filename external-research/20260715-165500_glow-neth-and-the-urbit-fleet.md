...
A Glow fleet lap is therefore a **composition**, not a new vane: Rhyz (membership) x Neth (settled status) x Pool (the app) x Comlink (carriage). This is the same compose-don't-name instinct that shaped Pool and the sandbox loop.
