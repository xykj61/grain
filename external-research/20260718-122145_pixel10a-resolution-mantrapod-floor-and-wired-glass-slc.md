...
Sources for an Acme employee to re-check: [Google Pixel phone hardware tech specs](https://support.google.com/pixelphone/answer/7158570) - store Pixel 10a specs page. GSMArena and press repeat the same 1080x2424 / 20:9 numbers.

**Checkable claim for LULU:** on this SLC body, panel max is 1080x2424 @ 20:9. Runtime `ANativeWindow` width/height remains the live oracle when a preset says "use panel max."
...
|-----------|-------------|--------|
| `panel_max` | 1080 x 2424 | Device max when body is Pixel 10a-class |
| `scaled_720` | 720 x 1616 | Same 20:9 (720 x 2424/1080) |
| `scaled_540` | 540 x 1212 | Same 20:9 |
...
**Design-provisional floor (until a panel SKU is sourced):** the seated Glow Glass layout word needs **>=32 Skate cols x 20 rows**. At minimum readable scale **2**:

- width >= `32 x 8 x 2` = **512** px  
- height >= `20 x 16 x 2` = **640** px  

That content budget must fit inside every LULU preset (letterboxed or pillarboxed if the panel aspect differs). For **20:9 phone presets**, the lowest table row that still clears 512x640 content is **`scaled_540` (540x1212)** -- height and width both above the budget. A future Mantrapod panel may be closer to square or 4:3; then the floor preset is re-derived from the same Skate budget, not from phone 20:9.

**Two Rooms:** budget math is checkable; "this e-ink SKU is NxM" is **not** claimed until sourced.
