...
**Seated beside auras `20260717.154943` -- ambient truth is Zig, not loobean.** Hoon/Nock's loobean polarity (0 = yes, 1 = no) is **not** Glow ambient truth. Glow conditionals and process exits follow Zig/`bool` and POSIX (exit 0 = success). Loobeans convert only at an explicit Nock/Hoon seam (`glow/truth_semantics.rye`). A future `@f` aura, if seated into the supplement, names that seam mold -- it does not redefine everyday Glow yes/no. Spec: [`../context/specs/20260717-154943_glow-truth-zig-ambient-nock-loobean-seam.md`](../context/specs/20260717-154943_glow-truth-zig-ambient-nock-loobean-seam.md).
