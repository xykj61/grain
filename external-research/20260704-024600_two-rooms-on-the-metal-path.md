...
| **Sovereign Brushstroke backend** | stack record section two backends | hosted backend green; framebuffer waits on kernel |
...
Bubble's harvest (*one log under every surface*) inspired Weave language already in the stack -- yet the full drawing-room product is not wired. The spark parked to RTAC; the vision kept its voice. **Lesson:** [`TWO_ROOMS`](../context/TWO_ROOMS.md) section doorway -- park the spark, do not let the image cite the bench prematurely.
