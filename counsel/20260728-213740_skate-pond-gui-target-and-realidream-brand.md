...
**Yes, with one sharpening.** The spirit is right. The seated words today already almost say it — this lean names the stack cleanly for Surface Season.

### What already holds

| Name | Seated today |
| --- | --- |
| **Skate** | Aspect of **Brushstroke** — text grid → pixel buffer ([LEXICON](../context/LEXICON.md) · graduation rule) |
| **Pond** | Bounded enclosure — packaging’s runtime target under policy; customs admit/hold/refuse |
| **Pool** | Applications **P-vane** — agents run *in* Pool; Pond is where they are enclosed |
| **Realidream** | Far-horizon **product surface** — editor + browser one face over one graph; Skate draws from that graph |
| **`.brush` → BrushFrame** | Description → Frame value (p23–p26); Skate/Wayland still later |
...
3. **Realidream is not “an instance of Skate.”** It is a **branded Pond/Pool application face** (editor+browser over one graph) that **instantiates Skate as its GUI target** — plus Mantra/Weave/Tablecloth under the graph. Skate is the general paint framework; Realidream is the special product that brands that stack. Dimeroll booksview · Mandi steward · Seva panels are sibling Skate consumers; Realidream is the horizon brand that unifies editor+browser.

```text
Pool agent (app)
    enclosed in Pond (customs · Tilak)
        describes surface (.brush → BrushFrame)
            paints through Skate (Brushstroke aspect)
                host backend (Wayland · NativeActivity · …)
