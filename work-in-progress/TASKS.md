...
- [ ] **Document the `RYE_ZIG`/`RYE_LIB` toolchain-selection footgun** — `rye build` silently falls back to any `zig` on PATH if unset, which mismatches the pinned std on any host with a different Zig installed
