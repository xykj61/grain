...
- [x] **Rish-native launcher + witness** — [`tools/cursor_jail_macos.rish`](../tools/cursor_jail_macos.rish) + [`tools/cursor_jail_macos_witness.rish`](../tools/cursor_jail_macos_witness.rish), landed `20260714.060500`; same SBPL profile as the bash elders (`join`/fixture-quote technique for the embedded `"` SBPL needs — a new `tools/fixtures/dquote.txt` one-byte fixture, same shape as the existing `newline.txt`), same fence, same `/dev/null` allowance; both witnesses green on this host; bash elders kept beside them, not replaced
