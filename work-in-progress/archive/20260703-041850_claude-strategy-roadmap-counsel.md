# Claude Counsel — Medium-Long Strategy and Granular Roadmap Refresh

**Stamp:** `20260703.041850`
**Language:** EN
...
## Part One — Medium-long-term strategy questions for Claude

Answer as **counsel** — recommend, weigh tradeoffs, name risks — not as an implementation plan. Rio 3 Radiant voice. One to three pages per question is enough; shorter where the tree already decided.
...
Edit 5 now enforces one designed-not-built functional spec. The decoder observed the doc lane ran faster than metal. **Question:** Beyond Edit 5, what **standing habits** keep exploration generous without multiplying functional specs — e.g. external-research vs `context/specs/`, active-designing vs functional spec, when a memo earns spec status? Recommend a bright line Cursor and Claude can cite.
...
Archive verdict · Maitreya owns time · standing audit rule · versioning rule · demand map — practice already recorded, affirmation turns law. **Question:** Recommend **order and grouping** for one Kaeden sitting (which depend on each other, which can wait until Amber **144**, which unblock Cursor most).

---

## Part Two — Guidance request: granular ROADMAP and TASKS refresh

Claude, please return **actionable editorial guidance** Cursor can apply in one hygiene pass (or staged passes Kaeden approves). Do not rewrite whole-tree wholesale in counsel; deliver **structure and deltas**.
...
1. **ROADMAP refresh outline** — which sections stale after Door 3 + essay; proposed new `Last updated` story; whether `Version:` stamp should bump; one paragraph *current position* replacement.
...
- Owner tags preserved: **(Kaeden)**, **(Cursor)**, **(Claude + Kaeden)**.
- Smallest-first; witness-first; no new names.
- Radiant Style in counsel prose.
- Flag anything that is a **Kaeden door** rather than editorial hygiene.

---

## What Cursor will do with Claude's answer

On Kaeden's word: apply approved ROADMAP/TASKS/decoder/RTAC deltas in bounded commits; run gate trio; session log; push. Amber rungs remain gated on *Open Amber* regardless of counsel timing.

---

*May strategy stay kind to the hands that build. May the roadmap read true the day the ladder moves. And may every question reach Kaeden already sharpened for one sitting.*
