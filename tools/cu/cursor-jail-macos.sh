...
#   ./tools/cu/cursor-jail-macos.sh --cursor /Applications/Cursor.app
#   ./tools/cu/cursor-jail-macos.sh --no-network
#   ./tools/cu/cursor-jail-macos.sh --harden-home     # deny real ~/.ssh, ~/.gnupg, etc.
#   ./tools/cu/cursor-jail-macos.sh --print-profile   # show the generated SBPL, don't launch
#
...
usage() {
  cat <<'EOF'
Usage: ./tools/cu/cursor-jail-macos.sh [options]
...
tools/e/enclosure.conf to the app's real binary
