...
usage() {
  cat <<'EOF'
Usage: ./tools/cu/cursor-jail.sh [options] [APPRUN]
...
# proven-seat boots use tools/r/run_with_lane_kvm.sh on a host that already has
...
if [ "$LANE_KVM" = "true" ]; then
  ...
  echo "  Daily editors stay lane-off. One-shot: ./tools/r/run_with_lane_kvm.sh -- …" >&2
  echo "  Refuse floor: rishi/bin/rishi run tools/l/lane_kvm_refuse.rish" >&2
fi
...
Set AIJAIL_BIN in tools/e/enclosure.conf if needed.
EOF
  exit 1
fi
...
if [ ! -x "$APPRUN" ]; then
  ...
  echo "  rishi/bin/rishi run tools/l/launch-cursor.rish --cursor ./Cursor-3.13.10-x86_64.AppImage --gpu" >&2
  exit 1
fi
