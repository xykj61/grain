...
#
#   cp tools/k/key-card.conf.example tools/key-card.conf   # then fill in your details
...
[ -f "$CONF" ] || { echo "No config at $CONF — copy tools/k/key-card.conf.example to it and fill it in."; exit 1; }
...
NY=160
P_GH="-312 488 -312 530 -312 770"; P_S2="0 488 0 530 0 770"; P_PGP="312 488 312 530 312 770"; FOOT=956
SQ="$W/square.png"

for k in gh s2 pgp; do convert "$W/$k.png" -filter point -resize ${QSZ}x${QSZ} "$W/${k}_r.png"; done
convert -background "$BG" -fill "$FG" -font "$FONT" -pointsize 15 -size ${CW}x caption:"$FP_SSH_GITHUB"  "$W/cap_gh.png"
convert -background "$BG" -fill "$FG" -font "$FONT" -pointsize 15 -size ${CW}x caption:"$FP_SSH_SECOND" "$W/cap_s2.png"
