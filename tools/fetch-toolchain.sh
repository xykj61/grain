#!/bin/sh
# tools/fetch-toolchain.sh -- the toolchain, in one command, before anything is built.
#
...
#
# Once Rishi stands, `rishi/bin/rishi run tools/fetch_toolchain.rish` does the same work with the
...
#
#   sh tools/fetch-toolchain.sh          # fetch, verify, extract
#   sh tools/fetch-toolchain.sh plan     # say what it would do; touch nothing
#
