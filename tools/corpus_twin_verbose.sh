...
# Source from a part-N resume, or:  source tools/corpus_twin_verbose.sh
...
twin_one() {
  ...
  timeout 300 "$E" run "tools/$s" "$@" >/tmp/e.o 2>/tmp/e.e; ee=$?
  ...
  timeout 300 "$N" run "tools/$s" "$@" >/tmp/n.o 2>/tmp/n.e; ne=$?
  ...
  if [ "$ee" = 124 ] && [ "$ne" = 124 ]; then v=SLOW-BOTH
    ...
    timeout 300 "$E" run "tools/$s" "$@" >/tmp/e2.o 2>/dev/null
...
prin_line "verbose twin_one seated — outer watch: source tools/prin_aliases.sh && pw"
