...
#
#   ./tools/l/lane_kvm.sh status
#   ./tools/l/lane_kvm.sh assert-off
#   ./tools/l/lane_kvm.sh assert-on
#   ./tools/l/lane_kvm.sh run -- <command...>
#   ./tools/l/lane_kvm.sh refuse-probe
...
usage() {
  cat <<'EOF'
Usage: ./tools/l/lane_kvm.sh <command>
...
Config: LANE_KVM in tools/e/enclosure.conf (see enclosure.conf.example).
  One-shot without editing conf: LANE_KVM=true ./tools/r/run_with_lane_kvm.sh -- <cmd>
  Full host sitting: rishi/bin/rishi run tools/l/lane_kvm_onpath_host.rish [--parity]
EOF
}
...
case "$cmd" in
  ...
  assert-on)
    if ! lane_on; then
      echo "REFUSE: LANE_KVM is off — set LANE_KVM=true in tools/e/enclosure.conf" >&2
      exit 1
    fi
    if [ ! -e /dev/kvm ]; then
      echo "REFUSE: /dev/kvm absent — relaunch from host with KVM granted (Pond customs later; one-shot: tools/r/run_with_lane_kvm.sh)" >&2
      exit 1
    fi
