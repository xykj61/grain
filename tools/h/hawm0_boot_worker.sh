...
# a caller like tools/h/hawm1_seva_witness.rish) -- treating it as a failure
...
if [ -n "${SERIAL:-}" ]; then
  ...
  echo "RED: emulator $SERIAL is attached but not fully booted (sys.boot_completed='$existing_boot') — close it first (tools/h/hawm0_stop.sh), then retry" | tee -a "$META" >&2
  exit 1
fi
...
# Stop it later with: tools/h/hawm0_stop.sh
trap - EXIT
progress "emulator left running (pid=$EPID serial=$SERIAL) — stop later with tools/h/hawm0_stop.sh"
