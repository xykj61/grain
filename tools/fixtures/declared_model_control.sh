#!/bin/sh
# tools/fixtures/declared_model_control.sh -- prove the model reading by doing.
#
...
#
# USAGE
#   sh tools/fixtures/declared_model_control.sh
#
# Driven by tools/m/declared_model_witness.rish. Run from the repository root.

set -u

scan=tools/fixtures/declared_model_scan.sh
read_one=tools/fixtures/declared_model.sh
...
runscan() { ( cd "$1" && DECLARED_MODEL_ROOT=. sh tools/fixtures/declared_model_scan.sh 2>&1 ); }
...
add "$pen/notmodel" "prose.md" "Run claude-code from .claude-state via launch-claude-season.rish."
...
if ( cd "$pen/norefuse" && DECLARED_MODEL_ROOT=. sh tools/fixtures/declared_model.sh model ) >/dev/null 2>&1
...
if ( cd "$pen/nokey" && DECLARED_MODEL_ROOT=. sh tools/fixtures/declared_model.sh model ) >/dev/null 2>&1
...
got=$( cd / && DECLARED_MODEL_ROOT="$pen/cwd" sh "$pen/cwd/tools/fixtures/declared_model.sh" model 2>/dev/null )
