...
sh tools/fixtures/reds_spine_grep.sh -i 'leans on a name|Crossing Chapter|working lean' >/dev/null "$COUNSEL" "$DUTY" || {
  echo "reds=failed"
...
rg -qi 'Crossing Chapter' "$DUTY" "$COUNSEL" "$ITINERARY" "$README" || {
  echo "lean=failed"
