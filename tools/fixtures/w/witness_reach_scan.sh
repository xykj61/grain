#!/bin/sh
# tools/fixtures/w/witness_reach_scan.sh -- which witnesses on disk does a runner actually run?
#
...
# tools/fixtures/r/room_bound_scan.sh reached for a terminal shelf.
#
...
#
# USAGE
#   sh tools/fixtures/w/witness_reach_scan.sh              # the seven readings
...
#   sh tools/fixtures/w/witness_reach_scan.sh --families   # the UNREACHED, grouped by name prefix --
#                                                        # the census follows the gate, so a family
#                                                        # count and the ceiling mean one thing
#   sh tools/fixtures/w/witness_reach_scan.sh --families-unheard   # the older reading, kept, since it
#                                                        # answers "has the choir been written yet"
#   sh tools/fixtures/w/witness_reach_scan.sh --outside    # self-declared witnesses the suffix
...
#
# Their three took their clocks the same evening, on the lap that measured the enclosure's own
# mounts: waymark_rung_drift and tally_bud at `lap`, counsel_census at `cadence`. So the debt the
# paragraph above names as another lane's is paid by its own lane too, and the ceiling falls again
# to whatever the two sets of rows leave -- measured, never assumed: 1107, six rows below 1113.
#
# 1107 -> 1108, and 219 -> 220, on the rebase that followed, and the one row is named rather than
# absorbed: tools/m/macos_app_bundle_witness.rish landed unrostered, opens a family of its own, and
# CANNOT take a clock on this pier -- it builds a macOS app bundle and dies at `app-bundle: the
# shell probe must build` on Linux. `host macos` is the seated tier for exactly that (REDS %295),
...
#
#   # reach-list: sh tools/fixtures/s/season_leaf_choir_scan.sh --list
#
