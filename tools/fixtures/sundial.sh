...
if rg -q 'outer.*paused|OUTER SCOPE \(paused' "$PROMPT"; then ok "prompt: outer paused"; else miss "prompt: outer paused"; fi
if rg -q 'i6 complete / i7 next|inner=paused\(i6/i7\)' "$PROMPT"; then ok "prompt: inner i6/i7"; else miss "prompt: inner i6/i7"; fi
if rg -q 'innermost.*u0|MUR Season held at u0|innermost=paused\(u0\)' "$PROMPT"; then ok "prompt: innermost u0"; else miss "prompt: innermost u0"; fi
if rg -q 'core.*g0|Gren Season held at g0|core=paused\(g0\)' "$PROMPT"; then ok "prompt: core g0"; else miss "prompt: core g0"; fi
if rg -q 'keeh_sponsor=Kia|Honor sponsor: Kia|Kia \(South Korea\)' "$PROMPT"; then ok "prompt: Keeh→Kia"; else miss "prompt: Keeh→Kia"; fi
if rg -q 'non-CVT|nonCVT|non_CVT' "$PROMPT"; then ok "prompt: used-mobility lean"; else miss "prompt: used-mobility lean"; fi
if rg -q 'Wayne_Hsiung|Wayne Hsiung|shyu_dedication=Wayne' "$PROMPT"; then ok "prompt: Shyu→Wayne"; else miss "prompt: Shyu→Wayne"; fi
...
if rg -q 'sundial' "$PROMPT"; then ok "prompt: sundial shortcut"; else miss "prompt: sundial shortcut"; fi
if rg -q 'gen_shyu' "$PROMPT"; then ok "prompt: gen_shyu named"; else miss "prompt: gen_shyu named"; fi
if rg -q 'gen_linn' "$PROMPT"; then ok "prompt: gen_linn named"; else miss "prompt: gen_linn named"; fi
