#!/bin/sh
# tools/fixtures/l/living_docs_roster_control.sh -- the roster-coverage guard, proven from both sides.
#
...
#
# Run from the repository root:  sh tools/fixtures/l/living_docs_roster_control.sh
set -eu

SCAN=$(pwd)/tools/fixtures/l/living_docs_roster_scan.sh
PEN=$(mktemp -d)
