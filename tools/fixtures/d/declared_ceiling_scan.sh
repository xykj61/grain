#!/bin/sh
# tools/fixtures/d/declared_ceiling_scan.sh -- a page that declares its own ceiling is measured
# against it.
#
...
#
# USAGE
#   sh tools/fixtures/d/declared_ceiling_scan.sh          # census -- key=value lines
#   sh tools/fixtures/d/declared_ceiling_scan.sh list      # every page, its declaration, its length
#
...
# tools/fixtures/l/living_pin_max_bytes.sh and every meter now calls it (REDS %199).
...
# number themselves. `tools/fixtures/l/living_pin_max_bytes.sh` is the one reading now, and this
...
if [ "$law_decide" -gt 0 ]; then
  echo "detail: $law_decide tool line(s) decide with a copy of the bound rather than reading tools/fixtures/l/living_pin_max_bytes.sh"
