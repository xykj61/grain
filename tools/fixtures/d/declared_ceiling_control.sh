#!/bin/sh
# tools/fixtures/d/declared_ceiling_control.sh -- prove the declared-ceiling meter from both sides.
#
#   sh tools/fixtures/d/declared_ceiling_control.sh
#
...
run_scan() {
  ...
  DECLARED_BOUND_OVER_CEILING="${2:-1}" DECLARED_BOUND_PROSE_CEILING="${3:-2}" \
    sh tools/fixtures/d/declared_ceiling_scan.sh census 2>&1 || true
}
...
reading=tools/fixtures/l/living_pin_max_bytes.sh
rp=$(mktemp -d)
