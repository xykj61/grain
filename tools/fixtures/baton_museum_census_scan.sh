#!/bin/sh
# tools/fixtures/baton_museum_census_scan.sh — eleven halls present; elders named.
#
...
need="counsel_close.brix agent_cover.brix recursion_prompt.brix tiered_handoff.brix cursor_relay.brix infusion_prompt.brix bench_apply_report.brix waymark.brix handback.brix season_summary.brix queue_packet.brix"
absent=0
for f in $need; do
  if [ -f "$root/$f" ]; then
