...
Work in math-angle theta, measured counterclockwise from the right (east). Convert to screen once, in a single helper, and let the y-flip live only there:

```
x = cx + r  -  cos(theta)
y = cy - r  -  sin(theta)          // minus, because SVG y grows downward
```

This yields a **south-up, east-right** frame -- the orientation the sky actually offers a watcher facing the pole:

- theta = 0 deg -> right -> **east** -> Ascendant / sunrise
- theta = 90 deg -> top -> **noon culmination** -> Midheaven
- theta = 180 deg -> left -> **west** -> Descendant / sunset
- theta = 270 deg -> bottom -> **midnight** -> Imum Coeli
...
Compute the daylight fraction from latitude phi and the Sun's declination delta on the birth-day:

```
cos(H0) = -tan(phi)  -  tan(delta)
daylight_hours = 2  -  H0 / 15
dayArc   = 360  -  daylight_hours / 24
nightArc = 360 - dayArc
```

The dayArc governs the whole figure. At the equinox it equals 180 deg and the wheel is symmetric; on a long summer's day it exceeds 180 deg; on a winter's day it falls short. The method handles all three with no special cases.
...
```
MC  = 90 deg                    // center of the 10th, straight up
IC  = 270 deg                   // center of the 4th, straight down
ASC = 90 deg - dayArc/2         // center of the 1st, on the horizon (east)
DSC = 90 deg + dayArc/2         // center of the 7th, on the horizon (west)
```

Here is the quiet geometry that makes it work: to keep the meridian upright *and* the horizon level *and* still show a long day, the horizon chord must sit off-center. When dayArc > 180 deg, ASC resolves below the right-horizontal, and the level horizon drops below the disk's center -- so the daylit segment above it becomes the larger one. The chord stays perfectly horizontal; it simply rides low. (At the equinox it passes through center; in winter it rides high.)
...
The only reliable source of error is the y-axis. Screen y descends while math theta ascends, so noon must map upward through the `-sin` term. Confine that inversion to the coordinate helper, verify the four angles land on their axes, and the wheel comes out true every time.
