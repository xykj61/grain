...
The five variants — **Reya, Riyo, Trey, Triz, Quin** — are one design seen five ways, so their docs are one template seen five ways. A single shared template, [`_variant-template.md`](_variant-template.md), carries a `{{VARIANT}}` token, rather than five hand-written pages that drift apart. Each variant's page is that template filled once. When the template changes, every variant page changes with it — the dual-description discipline (one source, many faithful views) applied to documentation itself.
...
Four of the five form two diverse-redundant pairs: **Riyo/Reya** and **Trey/Triz**. **Quin** is the fifth bootable image, **intentionally unpaired** — five total is the settled count. Redundancy *shape* for each pair (one codebase, two independently-verified paths) is tracked in [`../../../context/specs/20260713-235600_names-awaiting-confirmation.md`](../../../context/specs/20260713-235600_names-awaiting-confirmation.md). The template generates the per-variant pages (`reya.md`, `riyo.md`, `trey.md`, `triz.md`, `quin.md`) once the doc-fusion machinery (Scribble/Brix/Mantra) exists, rather than by hand today.
