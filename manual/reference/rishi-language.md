...
`map LIST as x: EXPR` transforms every element; `where LIST as x: PRED` keeps the elements that satisfy the predicate; `length LIST` counts. These three carry much of the parity suite. Branching and iteration with effects live in section 11.
...
**A field guide stands beside this contract** (`20260821.191504`). [`../../docs-geode/api/rishi-language-reference.md`](../../docs-geode/api/rishi-language-reference.md) answers *what do I reach for, and where will it surprise me* -- the string and list families in one table, and seven **edges** this page does not name: that interpolation takes a name rather than an expression, that there is no ordering operator, no newline escape and no multi-line string, that `slice` refuses rather than clamps, that a `let` inside `for-each` does not escape, that strings are bounded, and that `matches` is a reserved word. Read against each other this day, every specific claim on **this** page held -- including `.len` and `flag`, which the field guide had missed.
