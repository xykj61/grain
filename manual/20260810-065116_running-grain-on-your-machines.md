...
On any machine whose shipped `rye`/`rishi` binaries match its architecture, the shortest honest proof that Grain works is to run a witness:
...
**What runs today.** Clone the template and run a witness in the same breath:
...
The shipped `rye` and `rishi` binaries are built for exactly this architecture, so a `GREEN:` line is often the very first thing you see — no toolchain, no setup, no account.
