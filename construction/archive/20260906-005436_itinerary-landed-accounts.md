# ITINERARY -- landed accounts, folded `20260906.005436`

*Folded whole from the living [`../ITINERARY.md`](../ITINERARY.md) live front, every word kept.*

*Why this block folded.* It carried DIFFUSER's opening cold pass, the `%440` CLOSED account, the
682,425-pair routing-metric census, and three things the census named as buildable. Two of the
three are now built and rostered -- the zero-hop witness and the `route_hops` doc line, landed
`20260906.005436` under `%452`, which found the doc line was not merely thin but false. The
`agent_jail` environment fact in the same block is booked upstream as `%446`. What remains live
from it is one proposal, `seat_arc`, and it stands as a line on the card.

*Why a fold rather than a raise: the card stood 105 bytes from its 32,768 ceiling. Only a page
whose LIVING parts outgrew its number earns a new one, and what had grown here was a closed
account whose findings now live in a witness. Walk-back nib `41998f8835` in
