...
**Directives only.** This block held 50,477 bytes on `20260824.060012` -- 65% of the card -- because fifteen landed rounds had been written into it and left. A landed round belongs in *Prior laps* below, one line pointing at the session log that holds its detail. Walk-back checkpoint `7754ccf272`.
...
- **Roster cold, then hot.** Open the lap with `sh tools/fixtures/standing_equipment_run.sh` before touching anything, and run it again after `git add` so the green measures the tree the commit ships (REDS %174). **43 guards** stand; the roster is `construction/standing-equipment.kyri`.
...
**Git nib:** `e3f635a0f4` (**The page that named half its directory.** `image/README.md` stood at **400,042 bytes** -- the largest document in the tree -- and named **112** of the **227** `.rye` modules beside it, two of which live in `pond/apps/`. So 117 modules had landed unlisted: the whole Season G shape-analysis, selection, and scene-structure arc. Split into a **6,928-byte Door front page** and four pages beside it, with `MODULES.md` carrying **227 rows in twelve families**, each sentence from that module's own `//!` head. `image_module_roster` seats **43rd**, proven both ways on fifteen behaviors. The new Door page then read **27% against a 20% ceiling** on the lap it joined the meter, and swept to **13%**. **REDS %188, %189.**)

**Now.** The largest page in the tree is a door again. `image/README.md` fell from **400,042** bytes to **6,928**, and the 117 modules it never named now carry rows in [`../image/MODULES.md`](../image/MODULES.md) -- **227 of 227**, in twelve families, held to the directory by the 43rd standing guard (REDS %189). **Next, in size order, each its own round:** `lotus/README.md` **297,878** bytes over 240 `.rye` sources, the same split and the same roster question; then `crypto/README.md` **88,205** and `docs/CRYPTO.md` **68,547**, which carry one rung table between them and want reading together rather than apart; then `constel/README.md` **69,979** over 31 sources. `session-logs/README.md` **272,208** comes down by folding the logs rather than by splitting the page. **Standing beside them:** the index fold for the four rooms carrying 317 stale rows, each its own round since folding a room's index rewrites its front door; the ten pages `living_docs_lint` duty 4 names as wanting a Status line; and **21 `.rye` files in `image/` carrying non-ASCII in their doc comments**, measured this lap and left for a mechanical sweep. The class all of this belongs to is named in [`../active-designing/20260824-080208_the-roster-that-decides-what-gets-measured.md`](../active-designing/20260824-080208_the-roster-that-decides-what-gets-measured.md): the **DOOR roster** now names 8 documents against 34 module front doors, and the standing roster names 43 guards while 1,671 `*_witness.rish` files stand on disk.
...
**Seed cadence, standing at the gate `20260824.084007`.** The projection is current and its four gates hold on metal: `sow_project.sh` copied 6,948 files, scrubbed 1,065 and withheld 122; `sow_leak_scan.sh` reads **IDENT_CLEAN**; `tools/s/sow_witness.rish` reads **NO_PERSONAL**; `tools/s/seed_link_witness.rish` is GREEN. The last published projection is `fdaf8e3` from `20260824.003817`. **The force-push to `grain-os/grain` waits on Keaton's word** -- one line, `bash ~/grain/publish-seed.sh`, and it ships.
