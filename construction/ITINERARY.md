...
**Git nib:** `7b987463b7` (**Staging is not shipping.** The cold roster opened the lap at **42 guards, 39 green, 3 red**, where the previous lap's own open had read 41 of 41. One root, and it is a step rather than a gap: that round staged eleven paths and committed none. `readme_metrics` read **1669** witnesses against a fresh **1670**, `geode_libraries` read one short in five rooms at once, and `tracked_link` refused a living card citing an essay never added. Two of the three are exactly what `tools/hooks/pre-commit` exists to prevent -- armed, proven on throwaway repositories, and keyed to `git commit`, which the lap never reached. So the prevention was already whole, and the operator card now says a lap ends at the commit. **REDS %188. 42 of 42 green.**)

**Now.** Reds first, and this one closed on metal: three standing guards read red on the cold open because the round before had staged eleven paths and committed none, so `tools/hooks/pre-commit` never ran and two generated pages stayed one witness behind the tree. Repaired, shipped, and the limit named -- no guard can enforce that a lap commits, since it would have to run after the lap ends (REDS %188). **The crux is unchanged and now unblocked:** `image/README.md` at **400,042** bytes over 227 `.rye` sources is the largest page in the tree, and the survey is already run -- it names **112 of 227** modules, with two phantoms (`dexter_line`, `scooter_compose`) naming files that live in other modules, so 117 modules stand on the page's own floor unnamed. It wants the Caravan split: a Door front page, its reference prose lifted beside it, a complete roster table, and a guard holding that table to the directory. **After it, in size order, each its own round:** `lotus/README.md` **297,878** over 240 sources; then `crypto/README.md` **88,205** and `docs/CRYPTO.md` **68,547**, which carry one rung table between them and want reading together; then `constel/README.md` **69,979**. The class is named in [`../active-designing/20260824-080208_the-roster-that-decides-what-gets-measured.md`](../active-designing/20260824-080208_the-roster-that-decides-what-gets-measured.md), which leaves two tier-3 rosters measured and open: the **DOOR roster** names 7 documents against 34 module front doors, and the standing roster names 42 guards while 1,670 `*_witness.rish` files stand on disk. Standing beside them: the index fold for the four rooms carrying 317 stale rows, and the ten pages `living_docs_lint` duty 4 names as wanting a Status line.
