...
**Git nib:** `7f268a25fd` (**The mark law molts, and three queue rows close on a number.** `.claude/rules/stamp-and-name.md` goes **42% to 27%** in place under a checkpoint, its retirement section restructured to lead with what a stamp and a name give -- and the pass is **proven claim-preserving**, with zero numbers and zero paths dropped between old and new. **Three of four queued documents needed no molt at all** (2%, 0%, 19%), which only the measurement could say; a queue row is a question, and a number is a legitimate answer. Two reds rode along: the close-out carried `SHRED_PREP` past its bound, and the fold that fixed it changed one link's depth -- the very dimension the design essay named an hour earlier. **35 of 35 green.**)
