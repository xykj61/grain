...
**Git nib:** `ff843aaef9` (**Two rooms mirrored, and a molt queue opens.** `context/CHEMICAL_FORMULAS.md` writes the tree's operations as reactions, each family naming what it conserves. `spellbook/` and `biochemistry/` stand at depth 1 as **declared mirrors**, proven byte-identical with every link resolving from both homes -- which took rewriting every bare sibling link, the technique REDS %176 named one commit earlier. **Class Q** opens in the molt queue with four entries, each naming which molt it takes. `press/` ships **scrubbed** with three personal announcements withheld, since a name-change announcement is *about* the name. Links into `construction/` named in prose rather than linked, because that room is `personal` and every link into it dies in the seed. **35 of 35 green**, 1,437 documents reached, zero broken living links.)
