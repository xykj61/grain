...
7. **Reconciling the 36 drifted `.claude`/`.cursor` rule pairs** (REDS %194) -- 40 pairs stand, 4 agree, and the drift runs **both ways**: `.cursor/rules/reds-first.mdc` names a path that moved, while `.cursor/rules/send-word.mdc` holds an `ls-remote` pre-push guard the Claude side has never had. A bulk merge in either direction silently deletes a live safety rule, so each pair is a reading. `sh tools/fixtures/rule_twin_scan.sh diff <name>` shows any one of them; `rule_twin` (49th guard) holds the count under a ceiling that only falls.

Everything else -- design, code, witnesses, docs, weaves, seed *projection* (not push), reds -- is agent-doable and does not wait.

**Seed cadence, standing at the gate `20260824.112806`.** The projection is current and its four gates hold on metal, each read alone rather than beside a running roster: `sow_project.sh` copied **6,961** files, scrubbed **1,066** and withheld **122**; `sow_leak_scan.sh` reads **IDENT_CLEAN**; `tools/s/sow_witness.rish` reads **NO_PERSONAL**; `tools/s/seed_link_witness.rish` is GREEN. *Read alone* is now enforced rather than remembered: `sow_project.sh` takes a lock before it clears anything and a second projection exits 3 (REDS %193), because this same lap first measured `copied=1644` while the roster's own `sow` guard was re-projecting underneath it. The last published projection is `fdaf8e3` from `20260824.003817`. **The force-push to `grain-os/grain` waits on Keaton's word** -- one line, `bash ~/grain/publish-seed.sh`, and it ships.
