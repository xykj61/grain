...
**Git nib:** `c3b1f9b2f5` -- HEAD's parent, resolvable everywhere (%401).
...
**PER-FOLDER LOGINS DID NOT MAKE PER-SHIP SESSIONS** (measured `20260906.022430`): the pier holds exactly **two** credential files, both stamped `01:53`, neither touched since. `claude` reads `$HOME` whatever directory it runs from, so six per-folder logins are one login six times -- **copal's *already logged in* is that mechanism seen from outside**. A real per-ship session is made where `$HOME` differs, inside the jail: `./tools/ag/agent-jail.sh claude` from the tree, `/logout`, then `/login`.
