...
**Git nib:** `a962cb31f9` (**Kyri 6 receipts anything, and six beginner doors open.** `kyri/receipt.rye` generalizes the notation from session logs to any subject -- nine fields, a content address by SHA3-512, ten claims GREEN on metal with five refusals. **35th guard.** The name counts the voice lineage (Reya, Reya 2, Rio 3, Quin, Riyo, Kyri), a census rather than a forecast. Six foundations open beginner doors on **season, molt, Lattice, Scribble, Lantern, Ember**, all under the Door ceiling and linked from `context/KYRI.md`. A **Kyli** magazine piece on MOX ships in `press/`. **REDS %176** -- a mirror is byte-identical and a relative link is depth-dependent, so eight links broke while the guard stayed correctly green; the mirror moved to `ember-voices/` at depth 1 and the scan gained a third reading proven from both sides.)
