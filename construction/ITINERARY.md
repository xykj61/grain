...
**Git nib:** `9c59a9a852` (**The page that named half its directory.** `image/README.md` stood at **400,042 bytes** -- the largest document in the tree -- and named **112** of the **227** `.rye` modules beside it, two of which live in `pond/apps/`. So 117 modules had landed unlisted: the whole Season G shape-analysis, selection, and scene-structure arc. Split into a **6,928-byte Door front page** and four pages beside it, with `MODULES.md` carrying **227 rows in twelve families**, each sentence from that module's own `//!` head. `image_module_roster` seats **43rd**, proven both ways on fifteen behaviors. The new Door page then read **27% against a 20% ceiling** on the lap it joined the meter, and swept to **13%**. **REDS %188, %189.**)
