...
**Checkpoint `20260906.212046` -- walk-back nib `b25bee2b9d`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,921 of its 40,960 with **39 bytes free** while this lap held a recovered paper and four closed reds to report. *What moved, and it is only this seat's own prose:* the **DIFFUSER floor-paper account** condensed from 1,745 bytes to 1,676, naming both papers and keeping each finding's headline number and its grade. *What waits there, worth recalling:* the floor paper's own arithmetic -- the lattice ball at **575 at radius 7 and 833 at 8**, the sweep of **63,903** circulants of which **75.75%** already beat 14 at median **12**, `C720(1,8,75)` walking **9** on six edges, **720 = 6!** so star and pancake walk **7 on degree five** while bubble-sort walks **15**, and the reach gap that the sweep sees unit-generator circulants only. Every one stands whole in [`../external-research/20260906-152821_the-floor-you-can-stand-on.md`](../external-research/20260906-152821_the-floor-you-can-stand-on.md), which the card names and the research room's index now lists. *And a REDS fold was withdrawn WHOLE from this round, which is the seventh time this ledger records the shape:* this seat folded `%487`, `%489` and `%504` at `20260906.212046`; a peer took `%489` with `%508`/`%509` an hour earlier, and on the next rebase another took `%487`. Both peers published first, so under `%484`'s rule that one defect belongs to one record the fold withdrew rather than landing a one-row remainder -- and by then the peers' own folds had already carried the pin from 41,153 to **39,681**, under its bound, so the red the fold was built to close was closed without it. The withdrawn package stands by name at `refs/heads/pier/diffuser-20260906-212539`. *Named honestly:* no peer's live front was touched, and the room for this round's finding came out of this seat's own elder block first.

**Checkpoint `20260906.212057` -- walk-back nib `e567cbc128`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,976** of its 40,960 -- 16 over -- once this lap landed a parked round's Bakery account. *What was condensed:* the PETRICHOR block, whose `%512` detail this same lap folded to [`REDS-a-pen-one-path-wide-for-eight-ships-rows-512.md`](archive/REDS-a-pen-one-path-wide-for-eight-ships-rows-512.md) -- the one shelf this cairn named was split before it landed, so `%487` stands on [`its own shelf`](archive/REDS-a-handler-that-cleans-up-and-carries-on-rows-487.md) and `%508` on [`the lock shelf`](archive/REDS-a-lock-the-stamp-already-was-rows-489-508-509.md) -- and whose `%511` detail already stood on its own shelf. *What waits there, worth recalling:* the two-legs-two-failures reading that made `%512` a race rather than a flake, and the `66 -> 219 of 284` reach figure -- both whole in the shelves the condensed lines now point at. Petrichor's live question to Keaton -- 607 reachable pages, 82 leading nowhere -- is kept word for word. *Why a condense rather than a fold:* eleven cairns today already record this card at its ceiling, and the block condensed here is the one whose detail moved to a shelf in this very lap, so the pointer is newly true rather than newly thin.
