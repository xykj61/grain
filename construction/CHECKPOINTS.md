...
packet and session record. It moves whole through `itinerary_account_shelf.sh`; the live card keeps
one pointer, while every word and re-anchored link remains on the new shelf. No dated testimony or
peer account moves.
**Checkpoint `20260913.203500` -- walk-back nib `6ae1c2049`.** *Swept:* the elder BAKERY account beginning **THE RECEIPT READS HOW EVERY SOURCE MAY BE REACHED** moves whole from `construction/ITINERARY.md` to `construction/archive/20260913-203500_itinerary-landed-accounts.md`. *Why:* the live card reached 41,895 bytes against its 40,960-byte bound after this lap's account arrived. The elder block's root and dependency execute-bit proof remains whole on its shelf and in its `20260913.175721` session log; the live card keeps the newest Bakery account.

**Checkpoint `20260913.203000` -- walk-back nib `6ae1c2049`.** *Swept:* the rows stamped `20260912.172929` and `20260913.200226` leave the living pin together for `construction/archive/REDS-two-readers-that-needed-a-verdict-rows-727-728.md`. Both rows are CLOSED, and the new row puts the pin 163 bytes over its 40,960-byte bound. *Why:* both faults let a missing reading masquerade as evidence: one treated an absent search command as an empty population, and one treated an unfinished transcript as a receipt. The shelf preserves both rows whole; the pin keeps every OPEN row.

**Checkpoint `20260911.133037` -- walk-back nib `f7e78f51e3`.** `external-research/grain-lineage-silo/silicon-torus-hardware.md` graded **C 71** at Field before this lap, below the B door, so it molted IN PLACE -- a living path with 5 inbound citers keeps its name. Register was the whole of it: **7 negatives in 9 counted sentences, 77% against the Field ceiling of 30%**, where `xrefs 0 per 100w against 3` already sat inside budget. Five restatements, each keeping its claim exactly: *no core sits at an edge* became *every core sits inside the medium rather than on a rim*; *stand on ordinary geometry and do not require any contested physics* became *rest on ordinary geometry alone*; *do not depend on aether theory being correct* became *stand on geometry alone, whatever verdict aether theory eventually earns*; *not as a load-bearing premise* took Gauge's own *rather than* form, split into its own sentence; and *makes the strong parts stronger by not tying them to the contested parts* became *lets the strong parts stand on their own*. Every name, path, and figure carries forward byte for byte -- Cerebras, Wheeler, Dollard, the major-angle/minor-angle/radial-depth address, the `../../gratitude/grain-lineage/silicon_torus_hpc_hardware_proposal_whitepaper.md` link, and the Galaxy Pitch block. **The two remaining negatives stay on purpose:** `Ask: none` and `Scope: reading only -- no lap opened, no fabrication implied` are the format `.claude/rules/azimuth-galaxy-proposal-format.md` prescribes, and they are 2 of 10. The page reads **B+ 88**, register 20% of 10 sentences, reach 70 at grade 14. The elder prose stands at this nib.

`claim_preserve_scan.sh` reports two FAILs over the page and each was read by hand. **`require` 1 to 0** is one modality word leaving with the negation it belonged to -- *do not require any contested physics* says the same thing as *ordinary geometry alone*, and removing a negation is what a register molt IS. **The PROPER addition `Such`** is the sentence-initial capital the one split creates, rather than a name.

**Checkpoint `20260911.114108` -- walk-back nib `20bade17ea`.** `manual/guides/macos-ai-jail-setup.md` graded **C+ 75** at Field before this lap, below the B door, so it molted IN PLACE -- a living path with 10 inbound citers keeps its name. The whole of the repair is sentence splitting: reading grade **15 against the Field ceiling of 11** was the entire Reach penalty, where `xrefs 0 per 100w against 3` already sat inside budget. The largest single cause was the `**Versions, all enduring:**` header line, **one 179-word sentence** listing nine revisions, which becomes nine list rows carrying every word. Ten body sentences of 42 to 116 words split at existing colons, semicolons, and dashes. Every path, flag, error string, stamp, and proof claim carries forward byte for byte -- `sandbox-exec: sandbox_apply: Operation not permitted`, `Host key verification failed`, `Permission denied (publickey)`, `mach-register`, `mach-task-name`, the thirteen denied credential stores, `.git/ssh_config_jail`, `GLOW_PROFILE.bron`, and the `active-designing/yonder/date/20260714/20260714-184500_macos-full-private-home-design.md` link. The page reads **B 84**, register 26% of 91 sentences against the Field ceiling of 30%, reach 90 at grade 12. The elder prose stands at this nib.

`claim_preserve_scan.sh` reports two FAILs over the page and each was read by hand. **The eight PROPER additions** -- `All`, `Enumerating`, `Naming`, `Proven`, `So`, `Store`, `Suppose`, `The Rish` -- are sentence-initial capitals a split creates rather than names, and the one loss, `An`, is the same capital moving to `So` when *An agent already working* became *So an agent already working*. **`every` 15 to 14** is one enumeration made exact: *`core.sshCommand` ... plus `gpg.program` ... and `user.signingkey` -- every value set for you* became *sets three values for you*, followed by the same three named one per sentence. The quantifier ranged over exactly those three, so the count says what the quantifier meant.

**Checkpoint `20260910.155933` -- walk-back nib `423dd857af`.** `.claude/rules/placeholder-ship-names.md` graded **C 70** at Field before this lap and **C+ 75** after the register sweep, both below the B door, so it molted IN PLACE -- a living path with 27 inbound citers keeps its name. The whole of the repair is sentence splitting: reading grade **17 against the Field ceiling of 11** was the entire Reach penalty, and `xrefs 0 per 100w against 3` was already inside budget. Four paragraphs became seven, split at existing colons, dashes and semicolons. Every number, name, path and table cell carries forward byte for byte -- `~sampel-palnet`, `@ud` 1,624,961,343, the 256 prefixes and 256 suffixes, the five tier rows, `~bandun`, `~pacpet-solreb`, and the `../../PUBKEYS.md` link. The page reads **A 92**, register 23% of 21 sentences, reach 100 at grade 9. The elder prose stands at this nib.

`claim_preserve_scan.sh` reports two FAILs over the page and each was read by hand. **`every` 2 to 4 beside `none` 1 to 0** is one restatement moving a quantifier from the negative form to the positive: *None of these parse as valid `@p`* became *Every segment above runs to 4, 5, or more letters, and never to 3. So each of these refuses to parse.* **The PROPER additions** -- `Convention`, `Docs`, `It`, `So`, `To` -- are sentence-initial capitals a split creates, rather than names. Two further drifts the first draft carried were repaired rather than explained: a `may` that softened *docs that use* into *docs may use*, and a dropped `never to 3` that the surrounding claim implied and no longer said.

**Checkpoint `20260910.103008` -- walk-back nib `1c1490f62b`.** `tools/u/unheard_guard_witness.rish` carried a 39-line accounting trail beneath its `unnamed_population=461` equality: one paragraph per bump, seven of them, each naming the arrival that moved the count and checking the delta from both sides. The equality is replaced by a floor the scan carries, so the trail no longer holds anything the guard reads, and it condenses into one paragraph naming the seven values and the dates they spanned. Every paragraph stands whole at this nib. No number, path, stamp or verdict moved; the elder ceilings and their own trails are untouched.

**Checkpoint `20260911.024500` -- walk-back nib `a5832217d1`.** Recovering GRASS's lap of `20260911.013618` from the dead-letter box put its live-front account back on the card, and the card read 41,083 against its 40,960-byte bound. Nine phrases in that one block were condensed rather than a section shelved, since the block was forty minutes old and its elder shelf had already been written by the lap itself: `the fence line` dropped from the rota line, `240 reached, 18 not` read as `18 unreached`, `an exclusion thirty days old and measured` read as `an exclusion written`, `rather than a second closure` dropped, and three refusals compressed from `a foreign control reds the wall` to `a foreign control the wall`. Every figure, path, guard name, ledger number, and stamp carries forward unchanged -- 258 controls, 18 unreached, 11 orphans, 7 unrun, 75 legs, `%486`, `%626`. The elder wording stands at this nib, and the block's own shelf at `archive/20260911-013618_itinerary-landed-accounts.md` holds the account it replaced. The card reads 40,950.

**Checkpoint `20260911.063359` -- walk-back nib `87752825f6`.** Landing the stashed EARTH lap beside this AIR one put PETRICHOR's live front over the 40,960-byte ceiling by 553. The finished EARTH account -- the `crushed_index` title reading, its 48 control legs, and the three prose repairs that preceded it -- was shelved WHOLE with `tools/i/itinerary_account_shelf.sh` at `archive/20260911-063359_itinerary-landed-accounts.md` rather than condensed, and the card carries a one-line pointer to it. Two sentences of that block were tightened on the card before the shelf was cut, and both stand at this nib. The card reads 40,942.

**Checkpoint `20260910.081438` -- walk-back nib `a74c712240`.** Two front doors graded below B at Door and molted IN PLACE, since a living path with inbound references keeps its name. `mikrophone/README.md` read D 55 (register 40% of 10 sentences, reach 10 at grade 18) and now reads B+ 87 (register 4% of 21, reach 100 at grade 9); `glow/nock/README.md` read D 60 (register 31% of 22, reach 20 at grade 16) and now reads C+ 76 at Door and B 84 at Field (register 6% of 32, grade 12). Every edit is a restatement or a split at an existing semicolon, colon, or dash: no path, opcode, witness name, gate number, waymark, bound, or stamp moved. The elder prose stands at this nib. `claim_preserve_scan.sh` reports four FAILs over the pair, all on the modality axis -- `never` 6 to 5 and 2 to 1, `all` 1 to 2, `every` 6 to 7 -- and each was read by hand as an equivalent restatement: *leaks nothing it was never told to keep* became *holds exactly what it was told to keep*, and *never its execution floor* became *RISC-V through Zig is its execution floor*. Removing a negation is what a register molt IS, and the modality counter reads every such removal as drift.

**Checkpoint `20260910.062226` -- walk-back nib `de8132279f`.** `mandate/README.md` graded C+ 75 at Door and molted IN PLACE, since a living path with inbound references keeps its name. Twenty-three edits: thirteen restatements of a plain `not` into Gauge's own `rather than` form, and ten sentence splits made at existing semicolons. Every claim, path, proof name, bound, and stamp carries forward byte for byte -- no proof record, refusal contract, or landed date moved. The elder prose stands at this nib. The page reads B 82, register 19% against the 20% Door ceiling. Its remaining negatives are refusal contracts (`refused`, `never clamped`), which are honest and stay.

**Checkpoint `20260909.225124` -- walk-back nib `3e383939ba`.** COPAL's live front stood 895 bytes over its 40,960 ceiling after this lap's account landed. Two COPAL blocks shelved with `itinerary_account_shelf.sh` rather than condensed: the elder census account at `20260909-225010` and the landed U+2212 sweep at `20260909-225106`, the second replaced on the card by a one-line pointer carrying its elder chain. Both shelves hold every word; nothing was rewritten, and the card reads 40,154.

**Checkpoint `20260909.083100` -- walk-back nib `9e0b3bdc6c`.** Preserve Bakery's completed projection account with the shelf writer before replacing its live paragraph. Recover eleven saved shelf addresses from their original stash blobs; quoted drafts preserve unlanded claims without allocating ledger numbers. The three instrument rows already stand elsewhere byte for byte. The collision draft stays behind the published-number ruling. All stashes remain.
...
**Checkpoint `20260909.072646` -- walk-back nib `1fd73244ed`.** Recover Petrichor's parked demo package from stash `93a5246debe0`. The scan and control still match that stash's base exactly; restore its quotation handling and four checks. Replace the demo's August readings with newly run, explicitly dated examples, and recover its fifth check. Keep the study guide's later Bhakta introduction and name-reading door while removing the outdated population and citation claims. Update the two shipping indexes. The demo and study guide each read F 59 under judged Truth 50 before repair; their prior wording remains at this nib and every stash stays whole.
...
**Checkpoint `20260909.040052` -- walk-back nib `873786d833`.** Recover Copal's two parked logs and their account shelf from `204a697198`. Keep Petrichor's current seed description. The host-config section resumes the saved withholding finding in current prose. Recover the `gen_home` default and roster entry from stash 2; its C+ opening comment becomes plain Door prose while its checks stay intact. The saved comment and account remain on the parked branch and in all original stashes.
...
row. Both queues already record their completed readings; their removal from the
pin frees room for living work. Preserve the DIFFUSER account with the account
shelf writer. The research paper keeps its entire elder body beneath a fossil
banner; a fresh paper corrects recalled database claims using primary sources
and a refusal probe. No fossil is cut and no custody gate changes.
...
words. The old wording stays at this nib. Correct the inherited inline claim
that the Codex loop has a jail branch; its current script always runs bare.
Keep every executable shell line. This is a comment rewrite, with the two new
Codex scripts also receiving their executable modes. The DIFFUSER account on
`construction/ITINERARY.md` moves whole to a tool-written shelf so the card can
carry this repair and the seed export discrepancy found during the cold pass.

**Checkpoint `20260908.075953` -- walk-back nib `122af84631`.** *Swept:* `tally/parse_int.rye`, a living path with **149 living citers** that graded **C+ 75** -- register **50** on 8 sentences half of them negative, reach **50** at grade 14 against a budget of 9. A living name with that many inbound references keeps its name, so this is an in-place molt rather than a mutant. *What moved, and it is register only:* the module head, rewritten to lead with what the function does before what `std.fmt.parseInt` fails to do. Every claim stands word for word -- the silent `"007"` to `7` reading, the three things a leading zero warns about (truncation, a copy-paste seam, a number wearing another's shape), the check of every character before parsing, the widening only on a written request, the hex byte pair `"0a"`/`"0f"`/`"09"` as an honest leading zero, `.allow_leading_zero = true` standing at the call site, the gratitude to TigerBeetle's `stdx.parse_int` ported at the letter, and the `@field` indirection that lets one function reach the banned form while the ban holds elsewhere. **No code line moved**, proven by reading the diff with the comment lines filtered out: the one non-comment change in this round is a comment carried INTO this file, below. *What waits at the nib, worth recalling:* the elder head's own phrasing, which was negative because its subject is a refusal -- the second page this week to show a Door ceiling biting a passage whose topic is a denial, after `docs-geode/etc/README.md` on `20260908.035921`. The elder said its subject; the new one says its meaning, and the facts are identical. Grade **C+ 75 -> A+ 98**, register **50 -> 92**, reach **50 -> 100**, Truth and Service held at 100. *Named honestly, and it is the reach rather than the writing:* this file is the target of **20 tracked symlinks**, so one edit here is an edit in every room that links it -- the hazard `copy_sameness_witness.rish` names on every run, working exactly as intended. `tools/t/tally_parse_int.rish` and `caravan/abandon.rye`, which imports it across a link this same round created, are both GREEN after the molt.

**Checkpoint `20260908.052557` -- walk-back nib `8ec45a1fa2`.** *Swept:* `construction/ITINERARY.md`, which opened at **40,953 of its 40,960 -- seven bytes free** -- against a replacement account this lap needed to write, so a fold was arithmetic rather than a preference. *What went, and it is only this seat's own prose:* the elder **COPAL block** -- the ratchet that counted the population it was measuring -- **folded whole** to [`the 20260908.052557 shelf`](archive/20260908-052557_itinerary-landed-accounts.md), every word kept and its relative links re-anchored a level deeper. The row it names is CLOSED and its detail already stands on its own REDS shelf, so the card was holding the third copy. The fold is the first move rather than the trim, which is the `20260906.113014` cairn's own rule. *What waits there, worth recalling:* the `rule_twin` cohort reasoning the card no longer spells -- that one figure was answering two questions at once, *did a pair drift further* and *how many are there*, and only the second moved; the closed-day cohort file written as CONTENT rather than a git query, because history gets rewritten; and the reading that **8 of 11 pairs born since the seating arrived agreeing against 5 of 40 elders**. All of it stands on the shelf and on the REDS shelf it names. *One line that was not mine also left, and it is a fold rather than a condense:* the `%481` pointer, whose entire content was *this account is on a shelf* -- both its accounts were folded on `20260906` and the row reads CLOSED in the ledger, so the shelf it linked is reachable from `REDS.md` and from that shelf's own header. **No peer's live front, open question, or unlanded account was read or condensed**, and no number, path, stamp or REDS row moved. *Named honestly:* the card came back to **40,960 of 40,960 -- at the bound exactly, no bytes free**, so the next ship to write an account meets exactly the condition this one did, which is the standing bill this ledger has now recorded a dozen times rather than a fault of any lap that pays it.

**Checkpoint `20260908.035921` -- walk-back nib `c2a536069f`.** *Swept:* `docs-geode/etc/README.md`, a living path grading **D+ 69** at Door -- **5 of its 7 sentences carried a negative**, 71% against a 20% ceiling, and under the 8-sentence floor, so `prose_register_scan` counted the page unreadable rather than passing and no single sentence could have carried the reading across (`%430`). *What moved, and it is a restatement rather than a trim:* every claim stands in the new page -- the eleven genres named one by one, the second-page-earns-a-room rule, the waiting-area reading of a permanent `etc/`, and the room standing empty because eleven genres have covered everything shipped so far, which was checked against `ls docs-geode/` rather than taken from the elder sentence. Three framings were turned to face forward: *belongs on none of its shelves* became *fits outside all of its shelves*; *the answer is never nowhere* became *always has an answer*; *why it is empty* became *why it waits*. One structural move: the closing beat was split out of the bold paragraph that held it, since Reach holds bold-led paragraphs out of its word count, and Reach went **50 -> 80**. **The `**Status:**` line gained the `checkable room` token**, which with the `Molted` framing is the elder byte this cairn records as changed. *What waits at the nib, worth recalling:* the elder page's negativity was not carelessness -- the room's subject genuinely IS refusal, a page belonging to no shelf, and the page said its subject rather than its meaning. That is the clearest example this tree has of a Door ceiling biting a page whose topic is a denial, and the repair shows the topic survives the register pass intact. Grade **D+ 69 -> B+ 89**, shadow **A 94**, register **29 -> 100**, negatives **71% -> 0%** over 11 sentences, Truth held at 100 on three resolving paths. *Recovered rather than rewritten:* the repair was authored at `20260908.030459`, parked by a round-open into `stash@{1}`, and landed here by `git checkout stash@{1} -- <three paths>` -- so these bytes are that lap's, re-measured before landing.

**Checkpoint `20260908.022456` -- walk-back nib `d347eeca7d`.** *Swept:* `construction/REDS.md`, which stood **713 bytes** above its ceiling once `%360` gained a second reading nine days after its first. *What moved:* `%360` itself, the tree's oldest row and newly **BOOKED** -- two of every three guards heard by nothing, measured then and measured again. *What waits there, worth recalling:* both readings stand side by side in the folded row, and the pair is the point -- 115 roster paths against 247, reached 603 against 837, unreached 1,153 against 1,041 over a corpus that GREW. A single reading would have shown a large number and taught nothing.

**Checkpoint `20260907.191247` -- walk-back nib `2883599fe3`.** *Swept:* `mand/README.md`, a living path that graded **C+ 76** at Door -- 51 words carrying 4 links, so reach read **30** on a cross-reference density of 8 per 100 words against a budget of 1. *What moved, and it is an accretion rather than a trim:* the elder body was two sentences of navigation, and every claim in them stands in the new page word for word -- custody at the kernel seam, the three questions, and Linengrow hosting product commerce while Mand owns the rings. The page gained what it lacked: why the three rings arrive in that order, what they reward (a denial that leaves a trace, since one bare denial covers three separate situations), and a table naming the two modules this room reaches by symlink. **A 20260725 date line moved to 20260907**, which is the one elder byte this cairn records as changed. *What waits at the nib, worth recalling:* the elder page's brevity was itself the reading -- a door of pure links, which is what the Door setting's cross-reference budget exists to catch, and the before-and-after is the clearest example this tree has of that budget biting. Grade **C+ 76 -> A 92**, register **31% -> 6%** against a 20% Door ceiling.

**Checkpoint `20260907.160500` -- walk-back nib `45f4703b66`.** *Swept:* `construction/ITINERARY.md`, which stood at **41,440 of its 40,960** once a parked lap came back out of `stash@{0}` and its account landed beside a peer's newer one. *What moved, and it is this seat's own:* the **INCENSE** `%499` block, condensed on the pin to a pointer plus every question it carried, with the removed sentences held whole on [the `154440` shelf](archive/20260907-154440_itinerary-landed-accounts.md) under a heading naming the condensation. Its measurement -- eleven `pier/diverged-*` branches, ten never reaching main -- already stands on [its REDS shelf](archive/REDS-the-test-that-cannot-fail-rows-499-503.md) and on [`REDS-a-park-is-not-a-landing-rows-507.md`](archive/REDS-a-park-is-not-a-landing-rows-507.md), so nothing here is the only copy of anything. *What stayed on the pin:* every open question -- the shared-pen repair a lap cannot witness, `rule_twin`'s zero-slack ceiling, and `%499`'s own *rebase there rather than park?*, which COPAL asks one block above. *No peer prose was touched*, which is the difference between this cairn and the `20260907.154440` one it replaces: that sweep condensed COPAL's block, and the stash pop dropped it in favor of the newer COPAL upstream had published meanwhile. *What waits there, worth recalling:* the discriminator itself -- the merge-base equals the parent of the oldest commit in `xy/main..HEAD` exactly when the local line's base still stands upstream -- which is the one local test that would have spared this lap and COPAL's. *The card closes at 40,927 of 40,960.*

**Checkpoint `20260907.104723` -- walk-back nib `e75ed1ac55`.** *Swept:* `construction/ITINERARY.md`, which came off two rebases needing room for a doorway account. *What moved:* **GRASS**'s completeness-guard account and **COPAL**'s ordering measurement, condensed to pointer lines with every removed sentence held whole on [the `104723` shelf](archive/20260907-104723_itinerary-landed-accounts.md), which also carries GRASS's elder-shelf link the card no longer spells. Two peers' now-false readings were **corrected rather than condensed** -- Petrichor's *46 against a ceiling of 44* and Pheromone's *RED at 48 against 44*, both superseded by this lap's repair, each rewritten to point at it. *What stayed on the pin:* every question a condensed account carried -- GRASS's LADDER-vs-NAME reading, COPAL's `band` word and its LOCA simulator door. *What waits there, worth recalling:* COPAL's arithmetic that **the alphabet decides** which whole-tree gate a lap pays for, `exec_bit` answering at 1s where `shared_pen` lands at 1020s of a 1062s pass -- the measurement behind putting the eight zero-ceiling gates first at 116s, and the reason `%550`'s own sentence was corrected by its author. *Named honestly, and it is the rebases rather than the writing.* This lap first folded three accounts to a `101905` shelf and folded `%550` to a shelf of its own; **every one was withdrawn**, since BAKERY, GRASS, COPAL and PATCHOULI had each replaced their own block upstream within the hour and a peer had folded `%550` twelve minutes ahead of this hand. A published fold does not move, so the duplicate shelf was deleted. The row renumbered **`%557` -> `%560` -> `%561`** across the two rebases with its stamp holding every time. *And `%481` fired again, on this hand:* resolving the second rebase, a `git add -A` in the same command as the unmerged-file listing marked `construction/ITINERARY.md` resolved **with `<<<<<<< HEAD` and `>>>>>>>` still in it**, and the listing printed empty because the staging had already run. A `git grep` over the tree before the push is what caught it -- the check the `20260906.153148` cairn recommends, earning its keep. **Never stage in the same command that asks what is unmerged.** *The card closes at 40,943 of 40,960, seventeen bytes free.*

**Checkpoint `20260907.074815` -- walk-back nib `fe0a566e64`.** *Swept:* `construction/ITINERARY.md`, which stood **417 bytes** above its 40,960 ceiling after two grants landed on a card whose INNER LOOP section alone is 30,916 of it. *What moved:* the **vendor+seed laps C-H** booking, whose laps landed on `20260906` and whose shapes-and-traps page carries the full account. *What waits there, worth recalling:* the card row named gate `%1` as the step after the laps, which is a live custody gate and stays named in the gates section rather than only in a booking.

**Checkpoint `20260907.074611` -- walk-back nib `fe0a566e64`.** *Swept:* `construction/ITINERARY.md`, which stood **245 bytes** above its 40,960 ceiling once a lap booked to petrichor landed on a card already carrying only 147 bytes of headroom. *What moved:* the 493-byte **tools letter rooms** booking of `20260906.173141`, condensed to a pointer at its own brief, which already carries the seven-move split, the traps and the resolver rule in full. *What waits there, worth recalling:* the card's own text named which seven tools move freely and which need a hand -- the brief says the same and says it longer, yet the card's one-line summary of the split was the faster read.

**Checkpoint `20260907.045834` -- walk-back nib `9766e92d18`.** *Swept:* `construction/ITINERARY.md`, which stood at **41,065 of its 40,960** -- 105 over -- once this lap landed a parked commit's own card block on top of a card that had moved on without it. *What went, and it is only this seat's own prose:* PHEROMONE's two-row account for this round, **condensed** from nineteen lines to nine, with every removed word held whole on [`the 20260907.021249 shelf`](archive/20260907-021249_itinerary-landed-accounts.md) beside the account it already carries. Two pointer lines were **folded** there first, links re-anchored a level deeper -- the `002052` two-accounts pointer and the five-accounts recital -- which is the `20260906.113014` cairn's rule that a fold precedes a trim. *What waits there, worth recalling:* the sentences the card no longer spells -- that the parked commit renumbered its row **three times while its stamp never moved once**, which is the derived spine's fourth rule earning its keep rather than failing; that the permission `case` reaches **49 of its 95 files** outside the room the meter reads, two of them run by nothing in the tree; and that the elder prune took `.git` alone, so a permission whose one remaining file was a build-cache leftover read live. All three stand whole on the shelf and in the two REDS shelves it names. *Two other lines of this seat's own were shortened rather than folded, because each is still OPEN and belongs on the card:* the `%530` citation count, which keeps its number and its three lines and loses its narration, and the `%460` emulator question, which keeps every clause and loses four words. *No peer's live front was read or condensed at any point.* *Named honestly:* the 105 bytes were spent by a commit written four hours ago against a smaller card, so the overflow is the cost of landing a park rather than of this lap's own writing -- which is `%499`'s bill, arriving in a currency the row did not name.

**Checkpoint `20260907.023053` -- walk-back nib `467d743dde`.** *Swept:* `construction/SHRED_PREP.md`, which stood **1,678 bytes** above its 24,576 ceiling once the `.cursor` Class M row landed on a page already carrying two open sheds. *What moved:* the finished **Class M -- the vendor names** section, whose Codex and arbor rows are prepped and whose cut waits on the same word as the new one, folded to its own shelf. *What waits there, worth recalling:* the three banner exceptions -- spoken `.arbor` text, parsed catalogs, and a byte-pinned shell -- each with its own reason, which the next prep will want before it banners anything.

**Checkpoint `20260907.003623` -- walk-back nib `40431050cd`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,906 of its 40,960 -- 54 bytes free** against a replacement account of 1,632, so the elder block had to leave before the new one could be written. *What went:* this seat's own **DIFFUSER block**, **folded whole** to [`archive/20260907-001653_itinerary-landed-accounts.md`](archive/20260907-001653_itinerary-landed-accounts.md) -- every word kept, nothing condensed, and every claim in it landed: the routing paper, its witness, scan and control stand in the tree, the `BPEN` line carried from Petrichor was repaired in the same commit, and its fifteenth-firing question is a duplicate of the one merged onto the card at `20260906.212057`, which stays live there. *What waits there, worth recalling:* the elder paper's own arithmetic that the card no longer spells -- one walk from the identity IS the routing table at **719 entries, 3 bits, 270 bytes** against **194,130** per node, proven over all **517,680** pairs from all **720** sources with `wrong=0`; the circulant's ring-gap rule reaching **609 of 719**; and depths 1, 2 and 3 all totalling **4,786 hops** while depth-2 takes a different first hop on **162**. Every one stands whole in [`../external-research/20260906-195719_what-it-costs-to-decide-the-next-hop.md`](../external-research/20260906-195719_what-it-costs-to-decide-the-next-hop.md), which this lap's recovered paper names as its elder. *Named honestly, and it is why this cairn is not the one the fold first wrote:* the lap that performed this fold died at its own commit and the round-open stashed it whole. The lap that recovered it found the card moved by four peer commits, so **both byte figures were taken again on this tree** -- 40,906 and 40,896, where the elder draft read 40,952 and 40,917 -- and the shelf's own header records the same correction. No peer's live front was read or condensed, and no number, path, stamp or REDS row moved.

**Checkpoint `20260907.081259` -- walk-back nib `a3d5a2ade9`.** *Swept:* `construction/ITINERARY.md`, which reached **41,315 of its 40,960 bound** once this lap wrote a portability finding onto a card that opened with 46 bytes free. *What moved, and it is a fold rather than a trim:* my own `%541` account -- the fleet-call helper, its dry-run readings, its two stated bounds, and the three-stashes-in-one-day record -- folded **whole** to [`the 20260907.081259 shelf`](archive/20260907-081259_itinerary-landed-accounts.md), every claim word for word, with its one relative link re-anchored a level deeper. The account was landed as of this commit, so the fold is the first move rather than the trim, which is the `20260906.113014` cairn's own rule. *What stayed on the pin:* the three questions the account carried for Keaton, since an open question is not a landed account. *What waits there, worth recalling:* the two dry-run readings side by side -- **16 foreign across four trees** before this lap's portability repair and **24 candidates, 18 foreign across five trees** after it -- which together are the evidence that the `cd`/`pwd -P` resolver answers on real processes and not merely in a pen. *A correction this cairn should record:* the card briefly carried **156 `pkill` sites**, a number carried over from an unrelated line rather than measured; the grep reads **2 sites in 2 files**, and the corrected figure names its own repair on the pin. *No peer's live front was read or condensed.*

**Checkpoint `20260907.000549` -- walk-back nib `113339e6ec`.** *Swept:* `construction/ITINERARY.md`, one paragraph of 471 bytes, so this round's account could stand under the 40,960 bound. *What waits there, worth recalling:* the paragraph read *"`dated_path_witness.rish` reds at `refs_lost=166` against a ceiling of 165 -- and it reads 166 on the pre-lap tree too (`d9a10c18c`), so no lap added it. Only `dated_path_repoint` is rostered, so nothing has run the census guard; the scan prints no list, which is why nobody can name the reference."* **All three of its claims had been answered or overtaken on the same card.** The reading moved 166 -> **170**, in the block still standing above it; *nobody can name the reference* was answered whole there -- **139 backticked mentions, 15 bare, 15 links**, every one of the 30 promises in dated testimony; and *nothing has run the census guard* is false now, since `construction/standing-equipment.kyri` line 65 carries `guard dated_path` at `tier cadence`. *What only it held:* the pre-lap reading at `d9a10c18c`, kept here, since that is the evidence no lap added the drift. *Why a condense rather than a fold:* a stale claim on a living card is a repair, and there was no finished section of mine left to fold -- my own `%512` account went to a shelf in this same round.

**Checkpoint `20260906.223615` -- walk-back nib `bc37657e8f`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,688** of its 40,960 with **272** bytes of headroom, against two new blocks this lap needed to write. *What was condensed:* my own three landed BAKERY accounts -- `%516`, `%517` and `%510` -- from nine lines to five. All three were CLOSED and folded before this lap opened, so what stood here was already a pointer at two account shelves and two REDS shelves, and the condense keeps every one of those four links plus the live residue word for word. *What waits there, worth recalling:* the sentence *an instrument can perform its act correctly and still be wrong about which act it performed*, which is `%516`'s whole lesson in one line, and `%510`'s `unlanded` **2 -> 0**; both are whole on the shelves the condensed lines still point at. *Why a condense rather than a fold:* the accounts were folded already, and what remained on the card was the pointer itself -- there is nothing left to move, only a shorter way to say it.

**Checkpoint `20260906.212057` -- walk-back nib `e567cbc128`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,976** of its 40,960 -- 16 over -- once this lap landed a parked round's Bakery account. *What was condensed:* the PETRICHOR block, whose `%512` detail this same lap folded to [`REDS-a-pen-one-path-wide-for-eight-ships-rows-512.md`](archive/REDS-a-pen-one-path-wide-for-eight-ships-rows-512.md) beside `%487` on [`its own shelf`](archive/REDS-a-handler-that-cleans-up-and-carries-on-rows-487.md) and `%508` on [`another`](archive/REDS-a-lock-the-stamp-already-was-rows-489-508-509.md), and whose `%511` detail already stood on its own shelf. *What waits there, worth recalling:* the two-legs-two-failures reading that made `%512` a race rather than a flake, and the `66 -> 219 of 284` reach figure -- both whole in the shelves the condensed lines now point at. Petrichor's live question to Keaton -- 607 reachable pages, 82 leading nowhere -- is kept word for word. *Why a condense rather than a fold:* eleven cairns today already record this card at its ceiling, and the block condensed here is the one whose detail moved to a shelf in this very lap, so the pointer is newly true rather than newly thin.
**Checkpoint `20260906.212046` -- walk-back nib `bc37657e8f`.** *Swept:* this seat's own **DIFFUSER block** on `construction/ITINERARY.md`, replaced rather than condensed -- the elder floor-paper account left the card to make room for the round that recovered a stranded lap. *What waits there, worth recalling:* the floor paper's arithmetic -- the lattice ball at **575 at radius 7 and 833 at 8**, the sweep of **63,903** circulants of which **75.75%** already beat 14 at median **12**, `C720(1,8,75)` walking **9** on six edges, **720 = 6!** so star and pancake walk **7 on degree five** while bubble-sort walks **15**, and the gap that the sweep reaches unit-generator circulants only. Every one stands whole in [`../external-research/20260906-152821_the-floor-you-can-stand-on.md`](../external-research/20260906-152821_the-floor-you-can-stand-on.md), which the research room's index now lists -- the card's own trim to fit its bound dropped even the elder paper's name, so the index is where a reader finds it. *And TWO pieces of this round withdrew to peers who landed first, which is the seventh and eighth time this ledger records that shape.* A REDS fold of `%487`, `%489` and `%504` lost `%489` to one peer and `%487` to another across two rebases, and by then the peers' folds had carried the pin under its bound, so the fold withdrew whole rather than landing a one-row remainder. A sweep of nine shared scratch pens was landed upstream by Petrichor while this round ran; **one line of it survives as a correction rather than a duplicate** -- their sweep repaired `PEN` in `tools/fixtures/t/tlb_reach_census.sh` and left `BPEN` on line 113 constant, and BPEN is the build pen that actually deleted a peer's source mid-compile. Both withdrawn packages stand by name at `refs/heads/pier/diffuser-20260906-212539`. *Named honestly:* no peer's live front or repaired file was reverted; where upstream carried a repair plus a comment this seat's copy lacked, upstream's file was adopted whole and only the missed line re-applied on top of it.
**Checkpoint `20260906.212025` -- walk-back nib `e567cbc128`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,951 of its 40,960 with nine bytes free** while this lap held two findings. *What went, and it is a fold rather than a trim:* three lines whose entire content was *this account is on a shelf* were merged into one -- the `145647`, `140206` and `092312` landed-accounts pointers, standing three, twenty-four and seventy paragraphs apart. The row list the first one recited (`%467`/`%468`, `%459`/`%446`, `%450`/`%461`, `%445`/`%439`, the `040933` four, `%451` with `%469`) went with it, because **each shelf names its own rows in its own header**, so the card was holding a second copy of an index the shelves carry. *No peer's live prose was read or condensed*, which the [`20260906.113014`](CHECKPOINTS.md) cairn one page down rules is the move a cairn exists to refuse, and the [`20260906.150954`](CHECKPOINTS.md) cairn rules that two shelf pointers standing apart are one pointer that has not been merged yet. *What waits there, worth recalling:* nothing. Every row named stands whole in [`REDS.md`](REDS.md) or on the shelf its pointer names, and the three shelves are still linked by stamp from the surviving line. *Named honestly, because it is the same finding this lap booked:* the card came back to **40,957 of 40,960 -- three bytes free**, so the next ship to write an account meets exactly the condition this one did. That is [`20260906.211529`](REDS.md)'s shape one room over -- cited by stamp, since the spine has yet to bind its number -- a bound whose only backstop runs after the work has landed -- and it is left standing rather than paid for out of a peer's open account.

**Checkpoint `20260907.010121` -- walk-back nib `dfe160336a`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,905 of its 40,960 ceiling with **55 bytes free** while this lap held a finding of roughly 1,200. *What moved, and it is only this seat's own prose:* PHEROMONE's `20260907.001500` account -- the ratchet rostered at a ceiling its own tree already exceeded -- **folded whole**, every word kept, to [`the 20260907.010121 shelf`](archive/20260907-010121_itinerary-landed-accounts.md), with its one relative link re-anchored a level deeper. The row is CLOSED and its repair is described in full in its own session log, so the card was holding the third copy. The fold was the first move rather than the trim, which is the `20260906.113014` cairn's own rule. *What waits there, worth recalling:* the measurement that made that row worth booking -- a ceiling of 46 read on a 972-page tree and published seven hours later onto a 974-page tree reading 47 -- and the three withdrawals of that lap, which are the standing argument for a claim written at START. Both stand whole on the shelf and in `session-logs/date/20260907/20260907-002052_the-lap-that-withdrew-its-own-landing.kyri`. *No peer's live front was touched.*

**Checkpoint `20260906.210719` -- walk-back nib `e567cbc128`.** *Swept:* `construction/REDS.md`, which stood at 43,002 of its 40,960 ceiling -- **2,042 over** -- once this lap's row landed on a pin already carrying a full day of peer bookings. *What moved:* the three oldest BOOKED rows whose instances stand repaired, folded to one shelf under the clause they share. *What waits there, worth recalling:* each row's own three fields in full -- what went wrong, what caught it, what it taught -- since a fold keeps the pointer on the pin and the teaching on the shelf.

**Checkpoint `20260906.181513` -- walk-back nib `d720176514`.** *Swept:* nothing yet -- this cairn precedes a MOVE rather than a debride. Twenty-six files leave the `tools/` letter room their basename does not name, nineteen of them read by every fleet loop on its next lap, so the fleet was brought down on Keaton's word to make the move safe. *What waits there, worth recalling:* the tree as it stood with `fleet-loop.sh`, `fleet_lap.sh`, `fleet_watch.sh` and every seat prompt under `tools/l/` -- every launch line, paste, and dated log written before this stamp names those paths, and `tool_path_resolve.rish` is what recovers them.

**Checkpoint `20260907.004225` -- walk-back nib `4d33d9ef8d`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,912** of its 40,960 ceiling -- 48 bytes free -- while this lap held roughly 1,000 bytes of finding, so a fold was arithmetic rather than a preference. *What went, and it is only this seat's own prose:* three BAKERY accounts, all CLOSED and each already pointing at its own REDS shelf, **folded whole** to [`the 20260907.004225 shelf`](archive/20260907-004225_itinerary-landed-accounts.md) -- `%516`/`%517`/`%510`, `%499`/`%503`/`%501`, and `%515` -- replaced by one pointer inside this lap's own account. Every word reads at the shelf; nothing was condensed and no peer's line was read or touched. **The fold is the first move rather than the trim**, which is the `20260906.113014` cairn's own rule followed rather than arrived at the long way round. *What waits there, worth recalling:* the three residues the card no longer spells -- a third-reason enclosure skip, two refusal wordings that differ on this bench, a deadlocked pin reading `ok`; and `%515`'s own arithmetic, `cases_run` **36 to 54** and a rebase auto-merging two lawful commits **119 over a bound of 100**. All of it stands on the shelf and on the REDS shelves it names. *One citation moved with the sweep, and it is a fact rather than a trim:* the standing link-wall block cited its row as `20260906.223436` because the row was unshared when it was written; the anointed spine has since bound it, so it reads `%524` now -- derived-spine rule 4 releasing a citation at exactly the moment it was written to release it. *Also folded, one room over and for the same reason:* REDS row `%523` to [`a record that parted from the act`](archive/REDS-the-state-a-reading-cannot-see-rows-523-528.md), **BOOKED**, which bought the ledger pin the room this lap's own row needed. *Written for two rows and landed for one:* `%518` is the same organ and was folded by a peer an hour earlier, to [`true where it was aimed`](archive/REDS-true-where-it-was-aimed-rows-518-520.md), so the shelf was rewritten for the row that was still standing rather than landing beside theirs -- the shape five cairns below this one record, met again. *And this lap's own row renumbered on the same rebase*, because a peer bound `%529` at `20260907.002052` against this row's `20260907.003911`: the earlier stamp keeps the number, so this row took the next one, and every citation outside the ledger already spelled the stamp -- which is why the move cost one command rather than a sweep.

**Checkpoint `20260906.173444` -- walk-back nib `2909fcec4e`.** *Swept:* `construction/ITINERARY.md`, which stood at 41,931 of its 40,960 ceiling -- **971 over** -- after a rebase brought a peer's newer diffuser account and two of my own bookings landed on top of it. *What moved:* the oldest CLOSED accounts in the live front, whose rows already point at their own shelves. *What waits there, worth recalling:* each swept block's own numbers, which its shelf carries in full -- the card keeps the pointer and the shelf keeps the arithmetic.

**Checkpoint `20260906.153148` -- walk-back nib `8e86ead74f`.** *Swept:* nothing, in the end, and this cairn records why. *What was planned:* the card stood at 40,957 of its 40,960 bound with three bytes free, and this round carried two rows -- the parked work recovered, and the guard that called a park a landing -- so two of this seat's own already-CLOSED lines were folded to a fresh accounts shelf: `%467`/`%468` and `%451`/`%469`, each already carrying its own REDS shelf, so the card was holding a pointer to a pointer. *What actually landed:* the rebase brought a peer's fold of the same four rows, whole, onto [`the 20260906.134234 shelf`](archive/20260906-134234_itinerary-landed-accounts.md) at `20260906.145647`. Mine was redundant, so **the shelf this lap wrote was deleted and the peer's kept**, and the card carries their recital line rather than my two. Room was made instead by trimming **this seat's own new block from 2,290 bytes to 1,696** and condensing three of this seat's own already-CLOSED pointers -- `%481` twice and `%479`/`%464` -- each keeping its lesson sentence and its shelf link. That is where the room should have come from first: the `20260906.113014` cairn's own rule, arrived at the long way round. *And `%481` fired a third time in the same send:* the second rebase auto-merged `construction/ITINERARY.md` without reporting a conflict and left `<<<<<<< HEAD` around the **Git nib** line, committed; `README.md` and `construction/REDS.md` carried the same. Caught by `git grep` over HEAD before the push, which is the check this cairn now recommends after any rebase that touches a living pin. *Named honestly:* a cairn describing a sweep that did not land is worse than no cairn, so this one says plainly that its sweep did not land. **The fifth time this ledger records that shape** -- two hands folding one account within the hour -- and the reason the card's fold still has no allocator the way its numbering now does. *What waits there, worth recalling:* nothing. The four rows stand whole on their own REDS shelves and on the peer's accounts shelf; no peer's live front was condensed at any point.

**Checkpoint `20260906.140859` -- walk-back nib `94c43d02f0`.** *Swept:* `construction/ITINERARY.md`, which stood 87 bytes over its 40,960 before this lap wrote a line, and needed room for two findings plus a pointer to a closed one. *What moved:* the `%479`/`%464` account, **folded whole** to [`the 20260906.140206 shelf`](archive/20260906-140206_itinerary-landed-accounts.md) with its links re-anchored, and replaced by a pointer. Both rows are CLOSED and each already stood on its own REDS shelf, so the card was holding a third copy. **The fold is the first move rather than the trim** -- the `20260906.113014` cairn's own rule, followed here: no peer's live front was condensed, and this seat's own closed `%494` account was cut to a pointer before anything of anyone else's was touched. *One correction rode along, and it is a fact rather than a trim:* the Sibling-finds line calling four uncontrolled `*_example_missing` verdicts an open Tablecloth find now names the commit where that work already stands written -- `cc1da84f7`, parked by a round-open and unlanded since `20260905`, which is `%495`'s own subject met in the card's own text. *What waits there, worth recalling:* nothing that is not filed whole. The folded account's live sentence -- *an OPEN row wants a claim, a seat and a stamp, at START* -- **stays on the card** in the pointer, and is asked in three other places besides; its mechanism, its three withdrawals and its renumbering history stand on the two REDS shelves it names. *A second living path molted in place under this same cairn:* the header comment of [`../tools/r/rye_harness_roster_witness.rish`](../tools/r/rye_harness_roster_witness.rish), which the QA card read **C+/79** after this lap's edits -- register 59 on 41% negative of 24 sentences, reach 80 at grade 11. What moved is the register and the sentence length only: two long semicolon-chained lists became short sentences, and four incidental negatives were restated affirmatively (*not one of their paths is written anywhere* -> *every one of their paths exists only while the harness runs*; *is never run* -> *is run by nothing*; *neither reaches* -> *each stops short of*; *a flag it does not know* -> *a flag it has never been taught*). The negatives that ARE the subject stay, since refusal is what a witness header is about. Every claim, number and path is held exactly, and the witness runs GREEN after the edit. It reads **B+/87** -- register 73, reach 100 at grade 8.
**Checkpoint `20260906.134401` -- walk-back nib `6284759978`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,948 of its 40,960 with **twelve bytes free** while this lap held roughly 1,300 bytes of finding. *What moved, and it is only this seat's own prose:* both `%481` paragraphs -- the conflict-marker account and its recurrence on `CHECKPOINTS.md` -- **folded whole**, every word kept, to [`the 20260906.133957 shelf`](archive/20260906-133957_itinerary-landed-accounts.md), leaving two pointer lines. The ledger row itself already stands on [`its own REDS shelf`](archive/REDS-a-marker-no-content-guard-could-see-rows-481.md). About 4,900 bytes; the fold was the first move rather than the trim, which is what the `20260906.113014` cairn four rows down asks for. *Named honestly, because it is the move that cairn refuses:* I then condensed Pheromone's landed `%488` recital to a pointer to buy the last 400 bytes, read that cairn, and **put every word of it back**, taking the room out of this round's own card entry instead. The finding's mechanism is whole in REDS `%500` and in this lap's log; the card carries the pointer. *What waits there, worth recalling:* nothing that is not filed whole. The `%481` account's own measurements -- one file in 15,681, 31 readings over 12 states and 12 repositories, the `--cached`-read-as-a-revision trap, and the three teaching files excluded by name -- stand on the fold shelf and in the REDS shelf beside it. *Three rebases later, the card needed 705 bytes more and took them from this seat's own prose again:* the **identity gap** paragraph, whose closing claim -- *`mantra_glow_tend_limb1` locks `Line` to three fields, a seam, so it waits* -- this very round proved false, so it was rewritten to point at `%500` rather than condensed; the **two records came back out of the box** account condensed to its shelf pointer; and the `%440` line to its count and its lesson. *And one fold was withdrawn:* a peer folded `%491` to `REDS-a-folds-output-hides-the-link-it-breaks-rows-491.md` while this lap held its own `...it-broke-...` shelf for the same row. Theirs published first, so mine was deleted and its recital line dropped -- the fourth time this ledger records that shape, and the reason the card's fold still has no allocator the way its numbering now does.

**Checkpoint `20260906.130800` -- walk-back nib `f94cd877f6`.** *Swept:* `construction/ITINERARY.md`, which arrived from the rebase at 40,954 of its 40,960 ceiling -- **six bytes free** -- so booking the program's six open laps needed room rather than restraint. *What moved:* the *Prior laps* section's two shelf-pointer lines condensed to one, and its **One row, on purpose** habit paragraph shortened to its rule. *What waits there, worth recalling:* that paragraph's parenthesis, which records that `TASKS.md` and `ROADMAP.md` fused into this card on `20260823.103804` and are pointer stubs now -- the one fact in it a reader of the fixtures that still name those files would need.

**Checkpoint `20260906.214931` -- walk-back nib `94cba0f71b`.** *Swept:* `construction/ITINERARY.md`, which stood **39 bytes** from its 40,960 ceiling when this lap's account came to be written. *What went:* this seat's own account, **folded whole** to [`archive/20260906-214931_itinerary-landed-accounts.md`](archive/20260906-214931_itinerary-landed-accounts.md) -- 1,147 bytes of Glow finding replaced by a 540-byte pointer. Nothing was condensed; every byte reads at the shelf. *Worth recalling:* the account names why the row `20260906.195208` took five numbers, which is the price of a park written down rather than argued.

**Checkpoint `20260906.195208` -- walk-back nib `5826c9be29`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,540** of its 40,960 with **420 bytes free** while this lap held roughly 1,400 bytes of finding, so a fold was arithmetic rather than a preference. *What went:* one block, this seat's own, **folded whole** rather than condensed -- the landed `%506`/`%463` account (*a proof two living pages cite, and nothing ever compiled it*) to [`the 20260906.195208 shelf`](archive/20260906-195208_itinerary-landed-accounts.md), every word kept, replaced by a five-word pointer inside this lap's own account. No peer's line was read or touched. *What waits there, worth recalling:* the two readings the card no longer spells -- `walked` **2 -> 65** and `unwalked` **119 -> 56** across 63 Glow witnesses in one pass, and the planted `u32`-as-`[]const u8` that builds **exit 0** without the comptime walker and **exit 1** with it. Both stand on the shelf and in [the log that recorded them](../session-logs/date/20260906/20260906-153431_a-proof-two-pages-cite.kyri). The one live clause it carried -- *widening `rye_compile_reach_scan`'s claim predicate to living Markdown re-grades the whole `never` set at once, 17 files, 4 doc-named* -- is a sized-not-taken proposal that now stands only on the shelf, and it is named here so the next lap in this lane can find it without opening the card's history. *Named honestly:* five further trims of this lap's own new account followed the fold, from 1,655 bytes to 1,318, because a fold of 969 could not by itself hold 1,655; the card closes at **40,959** of 40,960. Every trim was to prose written this lap and never to a claim -- no number, path, stamp, or REDS row moved.

**Checkpoint `20260906.124443` -- walk-back nib `2b20991291`.** *Swept:* `construction/ITINERARY.md`, which stood at **42,293** of its 40,960 after two rebases brought four peers' live-front blocks in beside this lap's own two. *What went, and it is only this seat's own prose:* the `%488` residue account condensed from thirteen lines to eight and repointed at [its own shelf](archive/REDS-the-doubt-published-in-the-wrong-frame-rows-488.md); the dead-letter-box recovery from eight lines to four; the `%454` sky-distance account from nine to six; and the `%460` emulator block from five to five shorter ones. About 1,330 bytes, no peer's line touched. *What waits there, worth recalling:* the three measurements the card no longer spells. The residue block's own worked example -- the tenth unresolved site compiles into a pen it makes with `mkdir -p`, copying seven tracked `caravan/` programs in, every one of them reached by another build -- stands whole on the `%488` shelf and in [the log that recorded it](../session-logs/date/20260906/20260906-113330_the-doubt-published-in-the-wrong-frame.kyri). The `%454` block's torus pricing -- reachability bought at diameter 14 against the point reading's 5, and 132 isolated points going to zero -- stands on [its own shelf](archive/REDS-a-published-distance-and-the-road-it-promises-rows-454.md). And the recovery's two stamps, `20260906.060019` and `20260905.101545`, name the two restored logs themselves, which are now in the tree. *Resolved in the same pass, and it was not this lap's to make:* `construction/CHECKPOINTS.md` stood **published on `xy`** carrying an unresolved conflict marker at lines 3, 7 and 12 -- `%481`'s shape one file over, caught by the guard `%484` had just taught to read the index. All four rows on both sides of the marker are kept, newest first, because this ledger is append-only and every one of them is a real cairn.

**Checkpoint `20260906.120134` -- walk-back nib `2a3313c06b`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,793 of its 40,960 with **167 bytes free** while this lap held roughly 1,400 bytes of finding, so a fold was arithmetic rather than a preference. Three moves, smallest first. The two `%440` lines were **merged**: the card carried that row twice, once at the cold open and once at the rebase, and the elder's unique clause (*fifth through seventh across two laps -- the writer run by hand while the nib writer put the nib right twice with none*) rides in the surviving line, its count carried forward to eighth. Pheromone's landed `%466` retelling condensed to its pointer, keeping both lesson sentences (*the label was not softened -- the census was taught to see what it was mislabelling*, and *one resolver, two readers*) and the `never` **178 -> 64** and control **67**/**27** numbers; the row itself stands whole on the [shelf it names](archive/REDS-the-harness-the-census-could-not-see-rows-466.md). A third condensation was written and then **withdrawn on the rebase**: this seat folded Grass's landed three-row account from 2,475 bytes to a pointer line per row, and Grass had already folded the same account upstream, tighter, under its own `20260906.114347` cairn. Theirs published first, so theirs stands and mine was dropped rather than landing beside it -- the same shape three cairns below this one record, and the reason the card's fold still has no allocator the way its numbering now does. *What waits there, worth recalling:* the mechanism detail behind each condensed row, all of it filed -- `copy_lag`'s 108-against-themselves arithmetic and the `.gitignore:384` untracked evidence room, `%474`'s sixteen short links across five of eight shelves, `%475`'s one-variable proof on metal, and `%466`'s conditional-credit seam and its `asserted=3` reading that came back **0**. Each stands whole on the REDS shelf its pointer names, so what moved is the card's retelling and nothing else. *Named honestly:* the one condensation that landed is a peer's work rather than this seat's, taken because this seat's own live-front block is this round's finding and the card offered no landed BAKERY account to fold instead. The rebase then made the arithmetic moot -- upstream's own folds had already brought the card to 40,599 of 40,960 -- so what this cairn finally records is one merge, one condensation, and one withdrawal.

**Checkpoint `20260906.120134` -- walk-back nib `2a3313c06b`.** *Swept:* the header comment of [`../tools/u/unheard_guard_witness.rish`](../tools/u/unheard_guard_witness.rish) -- a **living path** with four living citers, so it molts in place under this cairn rather than leaving a mutant at a fresh stamp. *Why:* the quality-assurance frame, and the reading was already standing before this lap touched the file. Measured both blobs on one card: the HEAD blob reads **C+/78** (register 40, 60% negative of 25 sentences; reach 70, grade 12 against Door's 9) and the molted header reads **A/88** (register 63, reach 90). *What went:* the sentences whose negative form was **incidental** were restated affirmatively with every claim held exact -- *is not counted unheard* -> *falls outside the reading entirely*, *had no size for 36 days* -> *left its size unmeasured for 36 days*, *could not see at all* -> *stood outside*, *is never population* -> *stays outside the population*, *a claimless runner does not* -> *where a claimless runner stays out*, *the elder numbers do not move by one* -> *stand exactly where they stood*, *rather than silently fixed* -> *rather than quietly repaired*, and *a guard whose job is to name what nothing runs cannot be allowed to read its own findings* -> *a guard that names what stands unrun must stay outside its own reading*. Four long sentences were split, which is what moved reach 70 -> 90. *What stays, and why:* every negative that IS the subject -- the title line *a guard that is never run guards nothing*, *welcomes asserted as hard as refusals*, the four vacuum refusals, *WHAT IT DOES NOT REACH* (a section name this tree's guards share, so changing it here alone would break a convention to serve one number), and the standing *Purely local: no key, no signature, no network, no funds, no real device*. *What waits there, worth recalling:* nothing beyond phrasing -- no claim, number, path, stamp, or REDS row moved, the witness ran GREEN before and after, and every string the control asserts lives in `say` lines rather than in comments, so the pen reads the same script it read before.

**Checkpoint `20260906.114734` -- walk-back nib `c027ae5d2b`.** *Swept:* `construction/SHRED_PREP.md`, which stood at 24,432 of its 24,576 ceiling with **144 bytes free** while Tier 1 and Tier 2 of the vendor-name molt each wanted a Class M row. *What moved:* the finished `### Class M -- CUT 20260826` section, the rune-rename cut record, folded whole to [`archive/SHRED_PREP-rune-rename-cut-20260826.md`](archive/SHRED_PREP-rune-rename-cut-20260826.md) and replaced by a one-line pointer carrying its four numbers -- 19 renamed, 46 touched, 538 substitutions, 273 syllables standing. *What waits there, worth recalling:* the paragraph explaining WHY 273 occurrences of the syllable are the scope line rather than a leftover -- the rune keeps its spoken name -- which is the subtlest thing that section said and the sentence a future rename will want.

**Checkpoint `20260906.150954` -- walk-back nib `8e227fe95e`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,752 of its 40,960 with 208 bytes free while this lap held roughly 1,000 bytes of finding. *What went:* only this seat's own closed accounts, and by **merging pointers rather than condensing prose**. The `%497`/`%498` block -- six lines retelling an account already whole on [its shelf](archive/REDS-reach-and-claim-are-two-boundaries-rows-497-498.md) -- became a shelf citation, and that citation then merged with the `%488`/`%454`/`%464` pointer one paragraph down into a single *five Pheromone accounts closed on metal, all shelved* line naming both shelves. No peer's line was read or touched. *What waits there, worth recalling:* nothing. Each of the five rows stands whole in [`REDS.md`](REDS.md) or on the shelf its pointer names, and the sentence the `%498` block was named for -- *a count that cannot go below zero cannot tell you it is wrong* -- stands in that row's own text. The one phrasing the card loses is the `%497` clause *ten of eleven `unresolved` sites invoke no compiler*, whose numbers are in the row and whose measurement is in [`the log that recorded it`](../session-logs/date/20260906/20260906-141201_a-ceiling-that-rises-on-honest-work.kyri). *A second fold, on the rebase:* `xy/main` carried this seat's own `%488` account expanded back onto the card beside the pointer that had already shelved it -- the `%440` shape, an auto-merge resurrecting a folded block -- and the card read 41,065 of 40,960 once the peer growth above it landed. The eight-line account folded to the pointer it already had, `%488` rejoined `%454`/`%464`/`%497`/`%498` in one line naming both shelves, and the one clause that is a different fact -- *my own 41 orphans is WITHDRAWN*, which names the `090312` shelf rather than `%488`'s -- was kept in place. The row stands whole at [its shelf](archive/REDS-the-doubt-published-in-the-wrong-frame-rows-488.md), residue, controls, both pens and all. *And the number this lap booked moved:* `xy` published its own `%502` while mine sat unshared here, so mine renumbered twice, to `%504` under the derived spine -- the stamp `20260906.150954` is the key and never moved.

*Why a merge rather than a trim:* the `20260906.113014` cairn one row below already ruled that the fold is the first move and that condensing live prose to make room is what a cairn exists to refuse -- two shelf pointers standing three paragraphs apart are one pointer that had not been merged yet, which is a fold rather than a trim.

**Checkpoint `20260906.113014` -- walk-back nib `05c87d3d04`.** *Swept:* `construction/ITINERARY.md`, which this lap's `%485` account pushed to 42,338 of its 40,960 -- 1,378 over, and almost exactly what the account itself weighed. *What went:* nothing of anyone else's, and nothing condensed away. The account was **folded whole** to [`the 20260906.113014 shelf`](archive/20260906-113014_itinerary-landed-accounts.md), every word kept, and the card left holding a five-line pointer, the sentence the round is named for, and the one boundary that wants Keaton's word. The COPAL block it replaced -- the `%477`/`%478` account from the prior lap -- stands whole on [`its own REDS shelf`](archive/REDS-a-third-of-the-lane-and-the-stall-it-hid-rows-477-478.md). *What waits there, worth recalling:* nothing that is not filed whole. The mechanism, both plants, all five control legs, the discarded two-asker test and the leg that corrected the round are on the fold shelf; the three-field row is in [`REDS.md`](REDS.md); the guard's full reasoning and its cadence argument are in the `amphora_asker_reply` block of the standing-equipment roster. **The fold is the first move rather than the trim** -- a card over its bound is a page whose finished sections have not been shelved yet, and condensing a peer's live front to make room for one's own is the move this cairn exists to refuse.

**Checkpoint `20260906.101534` -- walk-back nib `151c129e83`.** *Swept:* the header comment of [`../tools/f/fleet_round_open.sh`](../tools/f/fleet_round_open.sh) -- a **living path**, so it molts in place under this cairn rather than leaving a mutant at a fresh stamp. *Why:* the quality-assurance frame. Measured both blobs on one card: the HEAD blob reads **C/74** (register 24, 76% negative of 13 sentences; reach 70, grade 12 against Door's 9) and the molted header reads **C+/75** (register 31, 69% negative, same 13 sentences; reach unchanged at 70). *What went:* six sentences whose negative form was **incidental** were restated affirmatively with every claim held exact -- *never misreport* -> *report each state by its own name*, *never sent* -> *stopped before the send*, *was NOT diverged* -> *stood exactly level with `xy/main`*, *No bytes are lost and no force is pushed* -> *Every byte is kept and every push stays fast-forward*, and the closing pair *it can no longer die / no longer lie* -> *it opens the round in every state it can meet, and names which one it met*. *What stays, and why:* the negatives that ARE the subject -- `RESET, NEVER MERGE` (the seated divergence word of `%290`), *never by force* (the twice-pulled rota's own law), *a round-open must never be able to fail on prose* (the reason the stash is wall-free), and *nothing ever emptied it* (`%464`'s finding). *What waits there, worth recalling:* nothing beyond phrasing -- no claim, path, stamp, or REDS number moved, and the control's asserted output strings live in `say` lines rather than in comments, so the pen reads the same script it read before. *Named honestly:* the molt was written by this seat's prior lap and recovered out of a round-open stash under `%477`; two ITINERARY-sweep cairns rode beside it in that stash and were **left unwritten**, because the sweeps they describe never landed and a cairn claiming a sweep nobody made is worse than no cairn. *What the frame did not close:* the header still reads below the B door on **reach** alone -- grade 12 against Door's 9, which is the operator card's own standing question about Door's ceiling against module heads running 12-17 -- so the register half is worked and measured, and the reach half is named rather than chased.
**Checkpoint `20260906.094500` -- walk-back nib `2d1c434a50`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,911 of its 40,960 with 49 bytes free while this lap held roughly 600 bytes of finding. *What went:* four settled lines, none of them a live question. The `%455 ANSWERED` block, whose whole content is that a jailed lap does write a roster receipt -- answered, and standing in [`the log that recorded it`](../session-logs/date/20260906/20260906-031132_the-window-closes-and-the-loop-takes-over.kyri). The `SEATED on the baton` line about regenerating `readme_metrics` after `git add`, which is now written into `tools/f/fleet_baton.txt` and [`the-baton`](../.claude/rules/the-baton.md) and reaches every ship in its prompt, so the card was holding a third copy of a seated rule. The one-line note that Patchouli's cold pass read the same two reds on a sixth tree, both of which the card's own later line records as closed. And the clause *Mantra's next work is MERGE, not `Line`* beside the `%439`-`%441` shelf pointer, which the card's own **THE WEAVE MERGES** paragraph has since superseded by landing that merge. The stale `20260906.053014` cold-pass reading was replaced by this lap's rather than removed. *What waits there, worth recalling:* nothing that is not filed whole. Each removed line is either a fact now standing in a rule, a shelf, or a session log, or a forecast the tree has since overtaken.

**Checkpoint `20260906.070005` -- walk-back nib `de5fe7068b`.** *Swept:* `construction/README.md`, the door onto the room that holds the operator card, molted in place rather than at a fresh stamp because the path is living and **449** living citers name it. *What went:* six claims that `TASKS.md` holds the granular plan and `ROADMAP.md` the standing order -- true until `20260823.103804`, when both fused into `ITINERARY.md` and became pointers -- plus a *Health (this sitting)* line reading `ROADMAP`/`TASKS` current and the card ~56 KiB over a 24,576 bound it no longer has, and a *Finishing edge* line calling the bench Cursor-sole. **Read again after `00fb6b08e` landed `%451`'s repair:** the door scored **C+/77** at Door under the elder `measure()` and **B/84 both before AND after** under the meter that ships, which reads the bold-led paragraphs the elder one dropped. So the repair moved this page's **truth** rather than its register, and `truth_counted` scored **100 both times** -- path resolution is the countable half, and all six false claims lived in the judged one. The elder pair is kept here as the reading that was actually taken. *What waits there, worth recalling:* the health line's third clause, `ready-to-ask-claude.md` ~57 KiB Claude-era -- archive on word, which is still true (57,070 bytes, measured) and is carried forward rather than dropped; and the July hammock table, which stays in place untouched, its nine `Tip **N**` rows kept by the nib rule's own clause for landed rows inside a living ledger. Nothing in the departing body stands nowhere else: the fusion is recorded in both pointer stubs and in `MAP.md`, and the elder card is one `git show` away at the `20260823.103804` cairn below. *This door is the seventh living surface of the rose repair* -- `de5fe7068` reached six and not this one -- and it was authored on a lap that died before sending, recovered from its stash under `%464`; the README stood byte-identical at both nibs (sha256 `0484f74b05`), so either resolves the walk-back.*

**Checkpoint `20260906.052900` -- walk-back nib `6ae49c380a`.** *Swept:* `construction/ITINERARY.md` again, in the same send. The rebase onto `b2c6e735e` brought Incense's `%450` close and their two-loops finding into the live front beside this round's own block, and the card read 41,975 of 40,960. *What went:* the `%461` CLOSED account, already condensed once this send, folded the rest of the way to a single pointer. *What waits there, worth recalling:* nothing -- the row stands whole in [`REDS.md`](REDS.md) with its mechanism, its renumbering history, and its lesson sentence, and the fuller card retelling is in the `20260906.050855` cairn one row below.*

**Checkpoint `20260906.111301` -- walk-back nib `d3b5aedf09`.** *Swept:* `src/gate/README.md`, a **living path with 449 inbound citers**, so it molts in place under this cairn rather than leaving a mutant at a fresh stamp -- the W2 shape. *Why:* the QA card read it **C+/79** at the Door setting, its register 62 on 38% negative sentences against a 20% ceiling, and the paragraph responsible is the one this lap's own restored work added -- an honest correction written as four sentences about what is absent. *What went:* the register only. Every claim is held exactly -- four heard and four unheard, the Aurora three and the Caravan one named, `%479` cited, the elder sentence quoted verbatim -- and the prose leads with what stands rather than with what does not. *What waits there, worth recalling:* nothing. The measurement is whole in REDS `%480` on its own shelf and in [`the log that recorded it`](../session-logs/date/20260906/20260906-104112_the-box-counted-and-never-read.kyri), and the paragraph's own numbers stand unmoved in the page above the cairn.*

**Checkpoint `20260906.050855` -- walk-back nib `fce04fe9da`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,669 of its 40,960 with 291 bytes free while this lap held roughly 1,200 bytes of finding. *What went:* the `%461` CLOSED account, condensed from twelve lines to three. *What waits there, worth recalling:* nothing that is not filed whole -- the row stands complete in [`REDS.md`](REDS.md), including the mechanism (`printf '%s'` drops an unterminated last `PATH` entry, and on this pier that entry holds `sh`), the renumbering history through `%459` and `%460`, and the sentence the row is named for. The one line the card loses that stands nowhere else is the aside *I also closed that bound red independently before the rebase; your repair is wider, so I took it whole* -- a peer courtesy rather than a fact, and its measurement is in [`the log that recorded it`](../session-logs/date/20260906/20260906-040420_three-meters-wrong-about-their-own-ground.kyri).*

**Checkpoint `20260906.010402` -- walk-back nib `f1498bd7fc`.** *Swept:* `construction/ITINERARY.md`, which stood at 32,697 of its then-32,768 with 71 bytes free while this lap held a 732-byte finding. *What went:* **only this seat's own landed lines** -- the one-line `%447` bound-census result, and the elder DIFFUSER pair naming three buildables (`seat_arc`, the zero-hop witness, one doc line on `route_hops`) plus the outfit finding that `Address.of_planet(8,0,0)` encodes to 8 while `decode(8)` answers tier `galaxy`. 653 bytes; no peer's line was touched. *What the rebase then found, recorded rather than hidden:* both halves of the pressure had already gone. A peer raised the card bound to **40,960** in the same hour, and a second peer folded **the same elder lines** to the [`005436` shelf](archive/20260906-005436_itinerary-landed-accounts.md) -- and **built two of the three buildables** they named, `prove_route_is_never_blind` and the doc line, closing their own `%452`. So this row marks a fold that upstream had already made, which is what a cairn is for: the nib is where this seat's version of those bytes stood, and nothing was lost on either path. The outfit finding is now **measured** in this lap's own row -- 132 isolated points, 33.28% of ordered pairs -- and every elder buildable lives on in [`a-third-of-the-sky-has-no-road`](../active-designing/20260906-010402_a-third-of-the-sky-has-no-road.md).
**Checkpoint `20260906.010047` -- walk-back nib `41998f8835`.** *Swept:* `construction/ITINERARY.md`, which had 105 bytes of headroom when this lap opened and needed room for `%453`. Two moves, both narrow: the card carried the `agent_jail` account **twice** -- once as the booked `%446` row and once as an elder paragraph written by another ship before that row existed -- so the elder was merged into the row, keeping its one unique clause (*the guard says only witness RED, naming no instrument*); and my own `%360` entry from the prior lap was condensed to its two live numbers. About 600 bytes, and the card now sits exactly at 32,768. *What waits there, worth recalling:* nothing that is not filed whole elsewhere. The `agent_jail` paragraph's own measurements -- `bwrap --dev-bind / / true` and `unshare --user true` both answering *Operation not permitted* -- stand in REDS `%446`, which is OPEN and names the same host fact from the other side; the `%360` detail (eight witnesses GREEN in 11s, the fall of exactly 8, why the ceiling took 1,108 rather than the measured 1,094) stands whole in [`../session-logs/date/20260905/20260905-230526_the-vessels-get-a-listener.kyri`](../session-logs/date/20260905/20260905-230526_the-vessels-get-a-listener.kyri). The two condensations freed what `%453` needed; then two rebases brought peers' live-front edits in beside them and the card crossed its ceiling again, so **Petrichor's mark-law weave** -- LANDED `20260905.224930` -- was folded whole to a shelf. **That fold was withdrawn on the third rebase:** Patchouli had shelved the same account to [`archive/20260906-001820_itinerary-landed-accounts.md`](archive/20260906-001820_itinerary-landed-accounts.md) an hour earlier, theirs published first, and the card is repointed at the shelf that stands. The same shape this hand met one lap ago with REDS `%438`, and the peer's own checkpoint two rows below records it a third time in the same day: **the card's fold has no allocator the way its numbering now does.** A peer also raised the card bound 32,768 -> 40,960 in the same window, so nothing here was forced by a ceiling in the end. What landed is the merge and the condensation; the account itself never needed moving twice.*
**Checkpoint `20260906.003146` -- walk-back nib `ad0014bc7c`.** *Swept:* `construction/ITINERARY.md`. The round's own fold was superseded on the rebase -- Patchouli had shelved the same DIFFUSER and topology block to [`archive/20260906-001820_itinerary-landed-accounts.md`](archive/20260906-001820_itinerary-landed-accounts.md) minutes earlier -- so the duplicate shelf this checkpoint first named was dropped rather than landed, and what actually moved was smaller: the `%445` and `%439` shelf pointers merged into one, the three `20260905` landed-account pointers merged into one, and the round's own find seated in five lines rather than fourteen. About 380 bytes, to bring the card from 33,145 back under its 32,768 -- a ceiling raised to 40,960 upstream in the same window, so the pressure that motivated this is already gone and only the tidy remains. *What waits there, worth recalling:* nothing this checkpoint removed is gone -- every merged pointer names the same shelf it named before, and each shelf holds its account whole. The one honest loss is a phrasing: `%445`'s pointer read *the view moved twice and the key did not* and now reads *the view moved twice, the key did not*.

**Checkpoint `20260905.235749` -- walk-back nib `9c8cd4acbe`.** *Swept:* `construction/ITINERARY.md`, which stood at 34,086 against its 32,768 ceiling after Patchouli's lap and mine landed into the live front within one hour. My own two entries were condensed by 361 bytes, and two CLOSED accounts -- `%430` and `%438` -- folded whole to [`archive/20260905-235749_itinerary-landed-accounts.md`](archive/20260905-235749_itinerary-landed-accounts.md), leaving one pointer on the landed line. About 1,640 bytes. *What waits there, worth recalling:* `%430`'s measurement of what the register floor cost -- 122 pages still freed, 52 newly scored, 25 of them leaving the B door they had been passing on a blank vote -- and `%438`'s sentence that a class of red cannot be written down without raising a meter, since the record of an instrument is still instrument. Both ledger rows stand whole on their own shelves and only the card's retelling moved; `%438`'s sentence is the one my own `%443` extends one material further, where the instrument and the field are the same file.*
**Checkpoint `20260906.001601` -- walk-back nib `e8be41ead3`.** *Swept:* `construction/ITINERARY.md`, which crossed 32,768 three separate times in one send as three writers landed rows in the same hour, and `construction/REDS.md`, which crossed 24,576 by 5,280. The ledger folded rows `%439`, `%440` and `%441` -- two CLOSED and one BOOKED, which the pin's own key permits -- whole to [`archive/REDS-what-no-meter-was-reading-rows-439-441.md`](archive/REDS-what-no-meter-was-reading-rows-439-441.md), and the card condensed four accounts whose rows now stand on shelves -- `%438`, `%430`, `%441`, `%442`/`%443` -- to pointers keeping every lesson sentence. About 2,000 bytes across both pins. *What waits there, worth recalling:* the full retellings of Patchouli's scope-crossing commit and bakery's ripgrep-off-PATH pair, each on its own REDS shelf; nothing was condensed whose row is not filed whole elsewhere. A peer folded `%438` to a shelf eleven minutes ahead of this hand folding the same row to a differently-named one; theirs published first and stands, and the duplicate was withdrawn on the rebase rather than landing beside it.*

**Checkpoint `20260906.062055` -- walk-back nib `dfcb4303eb`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,926 of its 40,960 ceiling with 34 bytes free. This seat's own landed `%449`/`%463` census block was condensed to a pointer to make room for `%467`. About 1,500 bytes. *What waits there, worth recalling:* the census's own numbers as first measured -- 1,940 tracked `.rye`, 227 symlinks, 1,713 distinct, 1,535 reached, 178 not, split 41 planted specimens and 114 unrun tests in `rye/tests/` -- and the retraction that rode with them, where this seat called `diff.rye` type-checked off an importing probe and then proved that probe blind by planting a type error it walked past. Both stand whole in [`REDS.md`](REDS.md) row `%463` and in the log [`20260906-045552_the-census-that-answers-zero.kyri`](../session-logs/date/20260906/20260906-045552_the-census-that-answers-zero.kyri); only the card's retelling was folded. The 114 unrun tests in `rye/tests/` are the larger door and are still unsized.*

**Checkpoint `20260906.060043` -- walk-back nib `229e402220`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,831 of its 40,960 ceiling with 129 bytes free while `%467` and `%468` wanted a live front. About 1,900 bytes condensed: the `%461`, `%462` and `%450` blocks to pointers; Grass's `.rye` census block, superseded when `rye_compile_reach` reached the roster; the elder hot-pass reading, superseded by this lap's own cold pass; and the `%360` line, whose 1,094-of-1,108 a peer's landing had already moved to 674 of 1,093. *What waits there, worth recalling:* the `%462` retraction's own mechanism -- that `fleet-loop.sh` pipes into `stream_claude` and a shell forks a sub-shell inheriting the script's `argv`, so `ps` prints two rows for one loop on every ship, and `pgrep -P` is what settles it. That sentence is the reason a future re-arm guard must not be written against `ps` output; it is kept whole at [`archive/REDS-what-the-instrument-was-looking-at-rows-462.md`](archive/REDS-what-the-instrument-was-looking-at-rows-462.md). *Named honestly:* this round folded `%462` twice and withdrew both, peers having published `%450`+`%461` at `20260906.060246` and `%462` at `20260906.061311`; a published fold does not move. Its own rows renumbered twice, `%464`/`%465` to `%466`/`%467` to `%467`/`%468`, and folded to their own shelf because every other pin row is OPEN.*

**Checkpoint `20260908.030324` -- walk-back nib `d8019ca95d`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,958 of its 40,960 ceiling with 2 bytes free, so a new lap's finding had nowhere to land. *Condensed:* the INCENSE block's account of the doorway-ratchet round (`20260907.192800`, BOOKED and shelved) and of the two `stash@{0}` reds (`20260907.180000`, CLOSED and folded) -- four paragraphs to two lines. *Worth recalling there:* the mechanism nobody had named for the doorway regrowth -- `external-research/README.md` recorded teaching the room token in its `Last updated` line while leaving its body silent, a door claiming to teach what it never said -- and the `mixed` token's own census, 334 of 846 door lines, which is why an essay reasoning from measurement toward a proposal answers with both registers. Both accounts stand whole in [`archive/20260907-192800_itinerary-landed-accounts.md`](archive/20260907-192800_itinerary-landed-accounts.md) and the folded REDS shelf.

**Checkpoint `20260906.040826` -- walk-back nib `61b71e4b04`.** *Swept:* `construction/ITINERARY.md`, which stood at 40,779 of its 40,960 ceiling with 181 bytes free. Two Pheromone `shell_dialect` lines were folded into one -- a peer's *item is closed* report about ITS tree, and this seat's elder naming of the two `sed -i` sites. About 260 bytes. *What waits there, worth recalling:* the `sed -i` sites in `fleet_key_locality_control.sh`:61,65 were genuinely repaired with temp-then-cat, and that repair stands; what the folded lines got wrong is that `shell_dialect` still reds on this tree for a DIFFERENT cause, measured `20260906.040826` -- the `shell_portable_control.sh` case *a real guard without its instrument names rg rather than a file* MISSes here, because its way of taking `rg` off PATH drops every PATH entry holding an executable `rg`, and on this NixOS pier `/run/current-system/sw/bin` holds `rg` and `sh` alike, so the scan under test cannot start at all. The card's live front carries that finding forward.*
...
seven living comment hits sweep in the same round (`rishi/src/main.rye`, `tools/c/chatgpt-mind.rish`);
...
`session-logs/date/20260826/20260826-044827_the-two-piers-and-the-nine-reds.kyri` and in rows %252-%254, so only
...
`session-logs/date/20260825/20260825-162410_the-ratchet-and-the-property.kyri` and in
...
`session-logs/date/20260825/20260825-132121_a-choir-for-the-largest-unheard-family.kyri` and in
...
decision paragraphs from `20260815`, the Compass Chapter's full SOON cell and JARL account and
...
**Why stubs rather than deletion, stated plainly.** Fifty-three tracked files read these three
cards, and most are **machinery** rather than prose -- roughly thirty `equinox_e*_scan.sh`
fixtures, the living-pin bound roster, the Radiant lint roster, `compass_rose.rish`,
`nib_honesty`, `seed_link`, `standing_equipment`, and `.brix` itself. A guard that *reads* a card
fails differently from a document that *links* one: the link shows a reader a 404, the guard shows
everyone GREEN while measuring nothing. Removing two cards outright would have asked thirty-nine
inbound references and a dozen witnesses to move in the same pass. The stubs keep every reference
resolving and every guard reading a real file, and retiring them is its own later round with its
own sweep.

**What waits at this nib, worth recalling:** the departing ITINERARY is **68,441 bytes** -- far past
the 24,576-byte living-pin bound its own header names -- and most of that mass is accreted *Now*
and *Prior lap* narrative from many rounds. The fusion keeps the live work-front and the standing
directives and folds the rest into pointers, so a good idea in an old lap's paragraph is one
`git show` away rather than in front of every reader forever.

### `20260823.100546` -- the name returns to Keaton Dunsford

**Walk-back nib:** `32d5bc0e45` -- **a deep debride**, so after the force-push this commit is
unreachable on both remotes. The pre-rewrite history survives in a verified 564 MB bundle at
`.debride-safe/pre-name-debride-32d5bc0e45.bundle` and in any clone taken before
`20260823.100546`.

```
git clone .debride-safe/pre-name-debride-32d5bc0e45.bundle recovered
```

**Swept:** the author and committer identity `Keaton Dunsford <xykj61@gmail.com>` becomes
...
**One honest note about the signature.** The signing key's UID reads *Keaton Dunsford (xykj61,
...
**Walk-back nib:** `fe951a4b9f` -- pier and both remotes. A **breach**, so nothing is rewritten.
...
**Walk-back nib:** `8ff548f937` -- pier and both remotes. A **breach**, so nothing is rewritten and
`git show 8ff548f937:construction/CHECKPOINTS.md` reads this ledger at its old name.
...
**Walk-back nib:** `d704942809` -- pier and both remotes. A **breach**, so nothing is rewritten and
`git show d704942809:manual/grain-os/README.md` reads any departing file at its old path.
...
**Walk-back nib:** `45e529d4b3` -- pier and both remotes. This is a **breach** rather than a debride, so
nothing is rewritten and the walk-back stays reachable everywhere. `git show 45e529d4b3:construction/ITINERARY.md`
...
**Walk-back nib:** `44ed7f7aa7` -- **and this one is different from every checkpoint above it.** This is
...
`.debride-safe/pre-debride-44ed7f7aa7.bundle` (559 MB, untracked, on this pier), and any clone
taken before `20260823.072824`.

```
git clone .debride-safe/pre-debride-44ed7f7aa7.bundle recovered   # the whole tree as it stood
git -C recovered log --oneline 44ed7f7aa7                          # browse it
```
...
**Walk-back nib:** `6e9c381f4c` -- pier and both remotes; nothing is rewritten, so this walk-back stays reachable everywhere.
...
amended in `context/LEXICON.md`, `construction/TASKS.md`, and `construction/ITINERARY.md`.
...
**Swept:** `construction/ITINERARY.md` -- the two full live-edge bullets for `caravan/reply.rye` (`20260821.071421`) and `caravan/edge.rye` (`20260821.063720`) condense into one pointer bullet, roughly 6 KB of card, as fold N takes the live edge. Both arcs are recorded in full in `caravan/README.md` and in their own dated session logs.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for **fold F**, folded into the condensed-pointer line beneath it so the card leads with the round that just landed rather than carrying a second copy of the log index. Roughly one screenful falls away; every word stands at the nib above.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for **fold D**, folded into the condensed-pointer line beneath it so the card leads with the round that just landed rather than carrying a second copy of the log index. Roughly one screenful falls away; every word stands at the nib above.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullets for the **allay** and **forbear** rungs, folded into the condensed pointer line beneath them, so the card leads with the rung that just landed rather than carrying a second copy of the log index.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **recount** rung, folded into the condensed pointer line beneath it, so the card leads with the fold that just landed rather than carrying a second copy of the log index.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **reweigh** rung, folded into the condensed pointer line beneath it, so the card leads with the rung that just landed rather than carrying a second copy of the log index.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullets for the **reopen** rung and for the **ladder-copy meter**, folded into one condensed line, so the card leads with the rung that just landed rather than carrying a second copy of the log index.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **deem** rung and the pointer bullet beneath it, folded into one condensed line naming deem, refer, swell, dwell, relent, heed, and endure, so the card leads with the rung that just landed rather than carrying a second copy of the log index.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **hear** rung, folded into the pointer line beside the courier, amend, and recant rungs the checkpoints below it swept, so the card leads with the record this lap seated rather than carrying a second copy of the log index.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **courier** rung, folded into the pointer line beside the amend and recant rungs the checkpoints below it swept, so the card leads with the hearing this lap proved rather than carrying a second copy of the log index.

**What waits there, worth recalling:** the inline account of a correction carried to the reader who never comes back -- the address seated on the wire beside the reading, the delivery landing outside this plan's own wire in `caravan/.readers/desk.told`, the four refusals of the journey named one by one, and the price stated as nothing but the walk. Every word of it rests in `caravan/README.md` and in the dated log `20260820-052038_caravan-courier-a-correction-that-travels.kyri`; the departing card reads whole at `git show c90bee22f7:construction/ITINERARY.md`.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **amend** rung, folded into the pointer line beside the recant, appraise, and bear rungs the checkpoints below it swept, so the card leads with the delivery and carries one account of this arc rather than five.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *live edge* bullet for the **recant** rung, folded into the pointer line beside the appraise and bear rungs the checkpoint below it swept, so the card leads with the amendment and carries one account of this arc rather than four.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *prior live edge* bullets for the **appraise** and **bear** rungs, folded into the pointer line the `20260820.014755` checkpoint opened, so the card leads with the recantation and carries one account of this arc rather than three.
...
**Swept:** `construction/ITINERARY.md` INNER LOOP -- the *prior live edge* bullet for the **mend** rung, folded into the pointer line the `20260820.014755` checkpoint opened, so the card leads with the appraisal and the bearing it stands on rather than three full accounts of one arc.
...
### `20260819.223443` -- ITINERARY's taper chronicle folded to a pointer as the entrust rung lands
...
### `20260819.221729` -- ITINERARY's unhand chronicle folded to a pointer as the taper rung lands
...
### `20260819.215403` -- ITINERARY's understudy chronicle folded to a pointer as the unhand rung lands
...
### `20260819.213726` -- ITINERARY's replace chronicle folded to a pointer as the understudy rung lands
...
### `20260819.211527` -- ITINERARY's withdraw chronicle folded to a pointer as the replace rung lands
...
### `20260819.205940` -- ITINERARY's enlist chronicle folded to a pointer as the withdraw rung lands
...
### `20260819.204716` -- ITINERARY's arrive chronicle folded to a pointer as the enlist rung lands
...
### `20260819.201448` -- ITINERARY's mask chronicle folded to a pointer as the precede rung lands
...
### `20260819.200013` -- ITINERARY's fence chronicle folded to a pointer as the mask rung lands
...
### `20260819.194255` -- ITINERARY's phases chronicle folded to a pointer as the fence rung lands
...
### `20260819.183640` -- ITINERARY's rolling and cohort chronicles folded to a pointer as the harvest rung lands
...
### `20260819.140253` -- ITINERARY's seven-ring boot chronicle folded to a pointer as the grant begins to do work
...
### `20260819.131316` -- ITINERARY's Microkernel-Target ring chronicle folded to a pointer as the fourth ring lands
...
### `20260819.125806` -- ITINERARY's Chapter-G scene-read chronicle folded to a pointer as the live edge moves to the Microkernel Target

**Walk-back nib:** `8d1b84c0ab` -- reachable in normal history (a working-tree condense, never a history rewrite); the full departing card is one `git show` away.
**Swept:** the **Live edge** bullet's whole Chapter-G scene-read chronicle (~5 KB of one run-on line) -- the four selectors named inline (`scene_manifest`, `hit_test`, `region_select`, `lasso_select`), the composing reads (`selection_summary`, `selection_bounds`, `object_relation`, `scene_graph`), and the entire line family rung by rung with its stamps and GREEN notes (`line_distance` through `line_corridor_summary`, `line_pierce`, `line_cross` and its rational meeting point, `line_raycast` and its slab-method entry parameter). The family stands whole and green at 227 image modules; the live front has moved to the Lindy-priority Microkernel Target, so the card keeps a pointer where the chronicle stood and refills Now with the live arc.
**What waits there, worth recalling:** at nib `8d1b84c0ab` the departing card holds the complete inline prose for the scene-read and line families -- the exact-integer reasoning, the `CrossNotWhole` refusal rather than a rounded intersection, and why `line_raycast` diverges from `line_slice` exactly where a wide box swallows a narrow one. No fact was lost: every rung carries its own dated log in `session-logs/` and a durable entry in `image/README.md`.

### `20260819.104554` -- ITINERARY's three giant prose run-ons (167 KB of a 197 KB card) folded to single-stranded pointers at the line-family set-close
...
**Walk-back nib:** `7884d47cae` -- reachable in normal history (a working-tree condense, not a history rewrite); the full departing card is one `git show` away.
**Swept:** the ~38-line wall of dead `<!-- prior nib -->` HTML comments in ITINERARY's INNER LOOP (the whole roundness quest -- roundness_universal, roundness, hit_test, scene_manifest, euler_scene, euler_number, and every earlier shape descriptor back through regions -- each a former live edge stacked as a comment), plus the ~4,600-char roundness_universal Git-nib paragraph, folded to short single-stranded pointers naming only the newest rung and directing to `session-logs/` for the per-rung detail. The card had grown back to ~187 KB, over its advisory bound; this is the same condense the `20260818.101851` checkpoint recorded, one boundary later.
**What waits there, worth recalling:** at nib `7884d47cae` the ITINERARY card holds the complete per-nib prose for the entire roundness quest and the earlier shape-descriptor family inline -- `git show 7884d47cae:construction/ITINERARY.md`. No fact was lost: each rung's crux, bounds, and metal-GREEN account is preserved verbatim in its dated `.kyri` session log, and `image/README.md` carries the durable per-module entries. The condense keeps the operator card the live card of what is next, not a second copy of the log index.

### `20260818.101851` -- ITINERARY Now condensed: the giant per-nib live-edge and git-nib paragraphs folded to pointers
...
**Swept:** the verbose `Prior Git nib` / `Older Git nib` / `Oldest Git nib` stack in `construction/ITINERARY.md` (lines 38-209), 81 giant per-nib paragraphs the loop had appended one lap at a time, condensed to a single pointer. The current `Git nib` (line 21, the loop's living code edge) and the lean `<!-- prior nib -->` HTML trail (the seven most recent, lines 23-36) both stay; the live status lines below (Host, crypto refinements, decision wave, latest lap) are untouched. Done on Keaton's word so the card stays single-stranded.
**What waits there, worth recalling:** at nib `d7e7694385` and every commit before it, the full stack reads whole -- `git show d7e7694385:construction/ITINERARY.md`. Every prior living-edge nib it named (the whole Chapter G image walk -- scrubber, text_subtitle, text_panel, shape, text_caption, text_reel, and dozens more) is a real commit recoverable by `git log --oneline`, and each rung has its own dated log in `session-logs/`. The trail moved to where git already keeps it.

### `20260817.171714` -- ITINERARY Today arc condensed (the done-work wall folded to a pointer)
...
**Walk-back nib:** `922e1f1a95`
**Swept:** *nothing yet* -- planted **ahead** of two newly approved breaches so each keeps its walk-back before it cuts: **Bron -> Kyri** (unify the notation entirely under Kyri -- `.kyri` takes the responsibility of `.bron`; Kyri is voice - notation - *compressed receipts* - preferred Grain variant, named in gratitude after Kyrie Irving) and **`work-in-progress/` -> `construction/`** (a higher-sorting priority folder -- `construction/ITINERARY.md`, etc.; 902 files reference `work-in-progress/`, every one repointed in the rename round). Each executes as its own signed loop round; the dated `.bron` logs' deep rename stays a separate circled step under the one-clock law.
**What waits there, worth recalling:** the whole tree under the elder folder name `work-in-progress/` and the elder notation name `.bron` -- every ITINERARY/CHECKPOINTS/TASKS path, every `.bron` session log, and the pre-rename reference graph. Walk back with `git show 922e1f1a95:construction/ITINERARY.md` or `git log --oneline 922e1f1a95`. Decisions + flags: [`../active-designing/date/20260815/20260815-175524_rye-first-crypto-parity-and-the-decision-wave.md`](../active-designing/date/20260815/20260815-175524_rye-first-crypto-parity-and-the-decision-wave.md).
...
### `20260809.024851` -- The Compass Chapter living-card debride
...
**Checkpoint `20260823.234815` -- walk-back nib `1d5ade54b7`.** *Swept:* `.claude/rules/stamp-and-name.md`, the mark law, molted in place into Gauge at its Field setting. The elder read **42% negative sentences** across 2,584 words, having accreted four amendments in three days -- the one-shape rule, the fold-by-what-it-is-found-by rule, the census-number exemption, and the illustration rule. *What waits there, worth recalling:* the amendments as separately-dated sections, each with its own heading and stamp, which is the shape that recorded HOW the law grew. The molt folds them into one argument and keeps every stamp inline, so the growth is still readable while the law reads as one piece. Every claim, number, path, and stamp carries forward -- 23 living citers keep their target, since a living path molts in place.
...
**`20260906.001820` -- Patchouli condenses the card's landed accounts (walk-back `9c8cd4acbe`).** *Swept:* `construction/ITINERARY.md` lines 110, 113, and 114-123 -- the three CLOSED paragraphs `%438`, `%430`, and `%441` -- about 3.4 KB, moved verbatim onto [`archive/20260906-001820_itinerary-landed-accounts.md`](archive/20260906-001820_itinerary-landed-accounts.md) and replaced by one pointer line. The card stood at 32,315 of its 32,768 bound with two reds to book. *What waits there, worth recalling:* `%430`'s measured effect on the 442 living pages (122 freed, 52 scored, mean fall 12.8) and the control leg that caught the repair on a path it does not take; `%438`'s lesson that a class of red cannot be written down without raising a meter; and `%441`'s module-room-reach numbers, which also stand in the row itself and in the essay's erratum. Nothing here is unique to the card -- each paragraph's detail is in its REDS row and its session log. *Extended during the same round's rebase:* four more landed accounts joined the same shelf when upstream's own live front arrived -- Petrichor's mark-law weave, the topology metric census, the peer's `%445` fold account, and the `%439`-class measurement -- each replaced by a pointer keeping its live clause, since the card had to hold two laps' fronts at once. The card read 33,819 of 32,768 at the worst point and 31,434 after. *Corrected on the second re-integration:* the REDS shelf this cairn originally named was dropped whole -- a peer published a fold of the same rows to `REDS-what-no-meter-was-reading-rows-439-441.md` in the same hour, and a published fold outranks an unshared one the way a published row number does. The ITINERARY shelf named above stands.

**Checkpoint `20260906.063431` -- the scope map's header molted in place (walk-back `0a2d848361`).** *Swept:* the twenty-five comment lines at the head of [`../tools/fixtures/s/standing_equipment_scope_map.sh`](../tools/fixtures/s/standing_equipment_scope_map.sh), rewritten at Gauge's Door setting. The elder read **register 62, reach 50** for a composite of **C+ 78**, below the B door, and the round was touching the file anyway to widen thirteen rows. *What waits there, worth recalling:* the elder head's own phrasings -- *"CURATED, NOT MECHANICALLY DERIVED"*, *"THE SAFETY DIRECTION"*, and *"The one unsound direction is a static row that names too little"* -- the last of which is the sentence that named this round's own red before anyone measured it, and is quoted forward in the new head rather than lost. Every claim, number, and stamp carries: the 20260829 survey of 111 rostered guards (59 static, 41 discovery, 11 env/clock), the absence-runs rule, the glob-and-room matcher semantics, and the build edge. Nothing here is unique to the file -- the elder head is one `git show` away at the nib above.

**`20260906.063431` -- BAKERY's card fold, withdrawn on the rebase (walk-back `0a2d848361`).** *Swept:* nothing, in the end. This round folded `construction/ITINERARY.md` lines 111-143 -- about 3.4 KB of landed BAKERY-lane accounts -- onto a shelf, because the card stood at 40,926 of its 40,960 bound with **34 bytes free**. While it was packaging, a peer condensed the same region in place and published it, with pointers to their own REDS shelves that the verbatim fold would have dropped. **A published condensation outranks an unshared fold**, the same way a published row number does, so the shelf was deleted and the peer's lines stand. *What waits at the nib above, worth recalling:* the elder full-length text of `%450`'s rule-pair reconciliation, the nib writer's four-file shape, `%462`'s retraction, and `%440`'s three firings -- each also whole in its REDS row and in the session log of the lap that landed it. Recorded rather than quietly dropped, because a cairn that names a fold which never happened is worth more than a silent one.

- `20260909.074555` -- Grass recovers two logs unchanged from stashes `1211d4c24a` and `2cd667aff2`. Their account shelves return from those same objects; the shelf-link repointer may correct inherited link depth, with the original bytes held in the stashes. Two forwarding addresses name a published identical row and a shelf cited under the log's own stamp. No historical log field changes and no stash is dropped.
