...
**Checkpoint `20260906.125308` -- walk-back nib `38431c7fe`.** *Swept:* the header comment of [`../tools/s/standing_equipment_witness.rish`](../tools/s/standing_equipment_witness.rish) -- a **living path** with ten living citers, so it molts in place under this cairn rather than leaving a mutant at a fresh stamp. *Why:* the quality-assurance frame, and it was standing before this lap touched the file -- HEAD's own blob reads **C+/77** (register 39, 61% negative of 39 sentences; reach 70, grade 12 against Door's 9), measured by copying that blob to a probe path inside the tree and removing it. *What went:* eight incidental negatives restated affirmatively with every claim held exact -- *can never name equipment the tree has moved* -> *every name in the roster stays one the tree still answers to*, *with an absent field meaning lap* -> *a bare row means lap*, *with nothing to say so* -> *each reporting itself as the only one*, *would tell a hand nothing* -> *would leave a hand exactly where it started*, *quiet without anyone noticing* -> *quiet while every pass still reads green*, *offered no number at all* -> *offered prose*, *and nothing else* -> *and stops there*, and *rather than merely to have passed* -> *rather than to have passed the same way twice*. One 126-word sentence was split into a named paragraph of four, which is what moved reach 70 -> 80. *What stays, and why:* every negative that IS the subject -- the title's *a list, not a sentence*, the law's *a guard that is never run guards nothing either*, *A fresh clone has genuinely run nothing*, *It reaps nothing: an orphan is still a live writer*, *this reading reports and never gates*, *No total is spelled here*, and *WHAT IS NOT PROVEN*, a section name this tree's guards share. *After:* **B/84**, register 57, reach 80. *What waits there, worth recalling:* nothing beyond phrasing -- no claim, number, path, stamp, or REDS row moved, every edit landed in comments rather than in an `assert` string the control reads, and the witness ran GREEN before and after.
