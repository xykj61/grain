...
- **Remotes -- the ordering remote and the mirror (seated `20260825.210819`):** `xykj61` is the **anointed ordering remote** -- the push whose fast-forward refusal is the round's compare-and-set -- and `origin` is the mirror, pushed only after `xykj61` accepts, never first (a mirror-first push is how two locally-perfect spines fork). A mirror refusal is a sync fault to repair, never a contested allocation. Design: [`../../active-designing/20260825-210819_the-round-that-pulls-twice.md`](../../active-designing/20260825-210819_the-round-that-pulls-twice.md).
- **The twice-pulled round (seated `20260825.210819`):** pull `xykj61` with `--rebase` at the round's open, BEFORE reading context, so the card read is the tree stood on; fetch it again after packaging and before the push; re-integrate at most three times behind jittered doubling backoff, re-verifying the witnesses whose watched files the round touched (full roster when touched sets intersect upstream's or a conflict lands off a registered fold surface); the third refusal parks the package on `refs/heads/pier/<name>` and defers by earlier round-open stamp -- never force.
