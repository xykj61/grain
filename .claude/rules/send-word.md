...
2. **Push** the working branch: `git push -u xy <branch>`, then mirror to `gp405` (retry on network error, exponential backoff).
3. **Merge to `main`**: fast-forward when clean (`git checkout main` - `git merge --ff-only <branch>` - `git push xy main` - then `git push gp405 main`), preserving every signed commit. Never force-push or amend unless Keaton says so explicitly.
