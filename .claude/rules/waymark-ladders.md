...
| **LOWE** | `cion-molt-living-mutants-and-fossils` | CION Equinox 2 — Molt (relabel living now-lines chronological/semantic). **Renamed from LOWE** (breach molt debride executed `20260817`, cairn `crux/CAIRNS.md 20260817.173615`, idx 2813); LOWE kept in the derive exclude as historical, and dated session logs + the REDS ledger keep LOWE as the label they recorded. |
...
| **DREY** | `season-a-mikrophone-forgetful-capture` | Season A second journey -- the **Mikrophone firmware** (the Mantrapod's near-term surface): the *memory that forgets* session buffer, proven pure in Rye before metal -- a bounded capture held only while powered, persisted only on a deliberate commit, provably dissolved on power-down. **Renamed from DREY** (breach molt debride executed `20260817`, cairn `crux/CAIRNS.md 20260817.173615`, idx 1189, ~502 sites: the `mikrophone/` module rungs DREY0..DREY16 and 17 `tools/drey_*` witnesses); DREY kept in the derive exclude as historical, and dated session logs + the REDS ledger keep DREY. |
| *breach round `20260817` -- all executed* | *cairn `crux/CAIRNS.md 20260817.173615`* | The five renames Keaton queued and then executed, each proven on metal and now seated as its own row above: **DREY -> DREY** (idx 1189, ~502 sites), **FORA -> FORA** (idx 1594, ~1027 sites), **LOWE -> LOWE** (idx 2813), **WADE -> WADE -> WADE** (WADE a transient dimeroll-era draw, re-breached to WADE when the module became Dimeroll), and the module **Dimeroll -> Dimeroll** (54 files). The elder marks (DREY, FORA, LOWE, WADE, WADE) stay in the derive exclude as historical; dated session logs and the REDS ledger keep whichever label they recorded, and are prepped for shedding on `crux/SHRED_PREP.md` under the explicit debride. |
| **FORA** | `constel-local-test-constellations` | **Constel** test-network naming — self-invented, siloed names for sandbox/testnet/localhost Comlink p2p constellations (fake ships, fake piers) run from inside the jailed pier, elder Urbit fake-galaxy dev nets as ancestor; consonant-only, structurally never a live-valid `@p` (segment lengths never 3), under the placeholder-ship-names law. **Renamed from FORA** (breach molt debride executed `20260817`, cairn `crux/CAIRNS.md 20260817.173615`, idx 1594); FORA kept in the derive exclude as historical, and dated session logs keep FORA. |
