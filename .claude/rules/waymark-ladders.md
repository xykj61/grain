...
| **LOWE** | `cion-molt-living-mutants-and-fossils` | CION Equinox 2 — Molt (relabel living now-lines chronological/semantic). **Renamed from LOWE** (breach executed `20260817`, cairn `crux/CAIRNS.md 20260817.173615`, idx 2813); LOWE kept in the derive exclude as historical, and dated session logs + the REDS ledger keep LOWE as the label they recorded. |
...
| **WADE** | `dimeroll-hr-and-accounting-entities` | Double-seated beside AHOY — DJINN's Bit Design System over the Skate·Realidream·Brushstroke surface (DVUI/Zig 0.16 in a Swift macOS Dock shell, Glow·Rishi·Rye·.brush impl) · Pond onboarding · Vultr SEA IaC · Dimeroll expansion for Siya Fund + Linengrow PBC HR & accounting. Name chain **WADE -> WADE -> WADE**: WADE renamed to WADE `20260817` (WADE drawn from the then-module name `...-dimeroll-entities`, idx 127); when Dimeroll renamed to **Dimeroll**, WADE was re-breached on Keaton's word to **WADE** (idx 5141) from the dimeroll-consistent preimage `dimeroll-hr-and-accounting-entities`. WADE and WADE both kept in the derive exclude as historical; dated session logs keep whichever label they recorded. |
...
| **DREY** | `season-a-mikrophone-forgetful-capture` | Season A second journey -- the **Mikrophone firmware** (the Mantrapod's near-term surface): the *memory that forgets* session buffer, proven pure in Rye before metal -- a bounded capture held only while powered, persisted only on a deliberate commit, provably dissolved on power-down. **Renamed from DREY** (breach executed `20260817`, cairn `crux/CAIRNS.md 20260817.173615`, idx 1189, ~502 sites: the `mikrophone/` module rungs DREY0..DREY16 and 17 `tools/drey_*` witnesses); DREY kept in the derive exclude as historical, and dated session logs + the REDS ledger keep DREY. |
| *breach round `20260817` -- five breaches executed* | *cairn `crux/CAIRNS.md 20260817.173615`* | Five **breaches** landed in the living tree, each proven on metal and seated as its own row above: **DREY -> DREY** (idx 1189, ~502 sites), **FORA -> FORA** (idx 1594, ~1027 sites), **LOWE -> LOWE** (idx 2813), **WADE -> WADE -> WADE** (WADE a transient dimeroll-era draw, re-breached to WADE when the module became Dimeroll), and the module **Dimeroll -> Dimeroll** (54 files). These are **breaches, not full debrides**: the living tree carries the new names, yet the elder marks still live in **git history** and the **dated session logs**. The full **debride** -- purging the elders from *all* history -- is a later **deep debride** (history rewrite + force-push, **custody gate #1**, Keaton's hand), not done here. The elders stay in the derive exclude as historical; the old-waymark logs are **molted** (prepped for shed) on `crux/SHRED_PREP.md` toward that eventual debride, the shred **RED** until circled. |
| **FORA** | `constel-local-test-constellations` | **Constel** test-network naming — self-invented, siloed names for sandbox/testnet/localhost Comlink p2p constellations (fake ships, fake piers) run from inside the jailed pier, elder Urbit fake-galaxy dev nets as ancestor; consonant-only, structurally never a live-valid `@p` (segment lengths never 3), under the placeholder-ship-names law. **Renamed from FORA** (breach executed `20260817`, cairn `crux/CAIRNS.md 20260817.173615`, idx 1594); FORA kept in the derive exclude as historical, and dated session logs keep FORA. |
