...
| **FORA** | *incoming, breach-queued `20260817`* | **Reserved replacement**, a **breach molt debride queued** on Keaton's word (cairn `crux/CAIRNS.md 20260817.173615`), not yet cut -- the retiring mark still labels its living sites until the rename runs: **FORA -> FORA** (`constel-local-test-constellations`, idx 1594), reserved in the derive exclude so nothing else draws it. **Executed `20260817`:** **DREY -> DREY** (now the DREY row above, idx 1189, ~502 sites); the module **Dimeroll -> Dimeroll** (54 files, ~1400 hits); **WADE -> WADE -> WADE** (now the WADE row above); **LOWE -> LOWE** (now the LOWE row above, idx 2813). |
