...
- **A checkpoint precedes the debride, never trails it.** For a *deep* debride (history rewrite + force-push), the checkpoint is mandatory and must land first — after the rewrite the walk-back commit is unreachable on the shared remotes. The Haunted Mound deep debride taught this by losing its own walk-back; that loss is why this pattern exists.
- **Append-only.** Checkpoints are never edited or removed — the whole point is a durable pointer. Newest first in the ledger.
- **A checkpoint is not a session log and not a remember.** `remember` reprints the *current* card; a checkpoint pins the *departing* one. Both may ride in the same send.

## Why the word exists

Debride is a clean, sanctioned break of accrete-never-break — yet a card that reads well often carried a stray good idea in a row we swept. A checkpoint costs one line and keeps every old card recoverable at a named commit, so the tree can cut freely without ever truly losing a thought.
