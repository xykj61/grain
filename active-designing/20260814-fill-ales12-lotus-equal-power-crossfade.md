...
> **The center holds power, not sum** -- at `i = den/2` each side is `isqrt(den2/2) ~= 0.707-den` (-3 dB), where a linear crossfade would give `0.5-den` (-6 dB). Two uncorrelated clips crossed this way stay level through the overlap.
