...
**Stamp:** `20260814.123000` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
...
Bounds are proven, not trusted: `max_pan_den` bounds the pan denominator, and `side_fader` asserts the fader and pan are within range so the folded numerator (`num × weight`) and denominator (`den × den`) never overflow the fader's `i32`/`u32`.
