...
**Stamp:** `20260814.112048` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
...
- `AudioStream` — a bounded byte FIFO (`max_stream = 2 × wire.max_frame`) with `push` (refusing `StreamFull`), `available`, and `read_frame` (one whole frame or `NeedMoreBytes`/`BadFrame`, cursor untouched on refusal); a drained stream compacts to the start on the next write.
- `Timeline` — a bounded ordered reassembly (`max_timeline = 4 × wire.max_samples`): the samples of every drained frame, concatenated, plus the frame count.
