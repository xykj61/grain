...
**SHAPE_XACT_DUAL.** Seat `shape-xact` / `cast-named-xact-shape` / `cast-import-xact-shape` so the first payload `$%` Glow OS desk leads with `xact-shape`. `mold-xact` and its cast twins stay. Allowlist alias from STOA152 stays. Capacity freeze held.
...
```bash
rishi/bin/rishi run tools/glow_run.rish glow/gen/shape-xact.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/cast-named-xact-shape.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/cast-import-xact-shape.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/mold-xact.glow
```

---

*May every payload Glow OS shape earn its dual seat beside the mold that taught it.*
