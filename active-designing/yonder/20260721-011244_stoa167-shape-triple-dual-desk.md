...
**SHAPE_TRIPLE_DUAL.** Seat `shape-triple` so the three-field Glow OS desk leads with `triple-shape`. `mold-triple` stays. Allowlist alias from STOA154 stays. Cast twins held (same hold as `shape-pair`). Capacity freeze held.
...
```bash
rishi/bin/rishi run tools/glow_run.rish glow/gen/shape-triple.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/mold-triple.glow
```
