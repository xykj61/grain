...
**SHAPE_COUNT_DUAL.** Seat `shape-count` / `cast-named-count-shape` / `cast-import-count-shape` so the second one-field Glow OS desk leads with `count-shape`. `mold-count` stays. Allowlist alias from STOA149 stays. Capacity freeze held.
...
```bash
rishi/bin/rishi run tools/glow_run.rish glow/gen/shape-count.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/cast-named-count-shape.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/cast-import-count-shape.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/mold-count.glow
```

---

*May every one-field Glow OS shape earn its dual seat beside the mold that taught it.*
