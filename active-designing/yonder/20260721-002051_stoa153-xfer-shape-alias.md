...
**XFER_SHAPE_ALIAS.** Seat `xfer-shape` on the closed gate-sample allowlist and on `is_xfer_named_mold` so bartis/barket lower emit the same XferMold identity path as `xfer-mold`. Capacity freeze at nine faces stays; `deca-mold` stays refuse. Mold remains the Hoon study word and today's desk suffix.
...
rishi/bin/rishi run tools/glow_run.rish glow/gen/gate-xfer-shape.glow
rishi/bin/rishi run tools/glow_run.rish glow/gen/gate-barket-xfer-shape.glow
```
