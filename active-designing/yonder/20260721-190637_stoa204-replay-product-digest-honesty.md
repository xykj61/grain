...
**DIGEST_HONESTY.** `claim_replay_demo_root` still double-replays via elder wrap, then calls `glow_sala_b0_digest.claim_digest_log` and refuses unless that root equals elder replay roots and the fixture. Fold and HAWM1 hex unchanged.
...
*May every claim name the digest surface that pins the root, while elder replay still earns the verify.*
