...
**1. Naked `_start`, then stacks.** The hart arrives with no valid stack. `_start` is `callconv(.naked)` -- no prologue before `sp` points at RAM. Each hart loads a base address and subtracts `hartid x 64 KiB` so stacks descend in parallel columns and never collide.
