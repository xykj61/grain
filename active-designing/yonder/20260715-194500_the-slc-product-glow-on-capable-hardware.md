...
| **L3 -- Audience & back-channel** | Many viewers; chat as folded signed facts; the VOD is the log at rest | A real sovereign alternative to a streaming platform, on your own phone | Broadcast -> audience -> replay, whole | **GREEN `20260715.210500`** (Sala B2) -- a mixed session (broadcaster content + viewer chat) replays multi-signer, each event verified against its own author's key; only consented voices fold (unknown signer refused); the VOD is the log at rest; two subscribers converge on one root |
