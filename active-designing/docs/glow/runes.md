...
The **27th digraph** (STOA111), tokenized, parsed, and lowered along the same ladder as bartis -- header-only (STOA112), identity (STOA113), call bodies (STOA115-116), the full named-shape allowlist, and one distinction of its own: barket passes `is_nesting=true` through the nesting-nests seam under stated shapes (STOA121/130), where bartis states the no-op. Header read whole; body pass shares round 3 with bartis's lower half. **Source:** [`rune_barket.rye`](../../../glow/rune_barket.rye) - [`lower_barket.rye`](../../../glow/lower_barket.rye).
