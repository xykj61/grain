...
| `secp256k1_ecdsa.rye` | a signature, hash, and public key — all public in verify | public-only — ECDSA **verification** reads only public values (r, s, z, Q, s⁻¹); it composes the variable-time scalar field and ladder over those *public* values, so it leaks no secret. ECDSA **signing** (a secret scalar and per-message nonce) is the custody gate and the **named horizon** | `secp256k1_ecdsa.rye` header |
