...
| `hkdf_sha512.rye` - `hkdf_sha256.rye` - `argon2.rye` - `pbkdf2_sha256.rye` - `pbkdf2_sha512.rye` | the input keying material / password | data-independent -- HKDF and PBKDF2 are HMAC calls (PBKDF2 a fixed `c`-deep iterate-and-XOR whose count is a public parameter, not a secret); Argon2's memory addressing is data-independent for the `i`/`id` (indexing) passes it is used through | `argon2.rye` header scope note |
...
| `eip155_tx.rye` | a transaction's fields, a signature, and a recovery byte -- all public | public-only -- the EIP-155 transaction **sighash** is the Keccak-256 of the RLP of *public* transaction fields, and `recover_sender` reconstructs a *public* sender address from *public* data (the sighash, r, s, and the recovery byte from v); it composes the GREEN RLP, Keccak-256, ecrecover, and eth_address over those public values, so it leaks no secret. A composition authoring only the EIP-155 nine-field assembly and the v<->(recovery_id, chainId) split -- data-independent -- so it sits on the non-gated side beside verify, ecrecover, personal_sign, and eip712; it builds the public preimage and recovers, never *sign this* | `eip155_tx.rye` header |
| `bitcoin_address.rye` | a serialized public key -- public data | public-only -- a Bitcoin address is HASH160 (SHA-256 then RIPEMD-160) of a *public* key, wrapped in the Base58Check (P2PKH) or Bech32 (P2WPKH) envelope; it composes the GREEN sha256, ripemd160, base58check, and bech32 over public values, authoring no arithmetic of its own, so it leaks no secret. Data-independent by construction -- it answers *what is this key's address?*, never *sign this*; it sits on the non-gated side beside `eth_address.rye` | `bitcoin_address.rye` header |
