...
|------|--------|----------------|
| **Witness suite** | `tools/p/parity.rish` | All **116** witness programs pass against pristine `std` (once per witness) |
| **Std overlay guard** | `tools/p/parity-selftest.rish` | `rye/lib/std` stays a symlink to vendor; tamper on a copied tree is caught |
| **Additive** | `tools/ad/additive-gate.rish` | If `rye/lib/` changes, only assertions/comments/`maybe` (guards against re-fork) |
