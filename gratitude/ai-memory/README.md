# ai-memory — study shelf (adopt nothing)
...
**ai-memory** (Fabio Akita / `akitaonrails`) is a Docker-backed daemon that holds session memory for coding agents. We study it beside sibling **`ai-jail/`** — same author family, different seam: jail is the unsigned authority we already run; ai-memory would be a second unsigned authority we do not seat.
