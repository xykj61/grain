...
Each writing turns a chat insight into a citable design artifact the roadmap can plan against. The strata doc sharpens the naming-mapping proposal's architecture before any rename PR is sized. The fleet doc names a real, composable Glow application lap (Rhyz x Neth x Pool x Comlink) and the design choice it hinges on. The mold-nesting doc names the sixth surface a future Glow supplement must cover, keeping the language-design thread whole. All three stay proposals, gated behind Keaton's word before anything is seated or built.
