...
#    onCreate runs JABS0 Seva fold; writes files/seva_root.txt (HAWM1 demo root)
...
**Surface + Skate on Pixel GREEN `20260717.125858`:** `glow_native_surface.rye` rasterizes Brushstroke Skate onto the ANativeWindow; Keaton confirmed on-device. Fold + `seva_root.txt` still land in onCreate; tap finishes.
...
**TUBE1 glass gate GREEN `20260717.131404` · cover `20260717.161823`:** `tube1_glass_gate.rye` admits seva-broadcaster (state resources + `photos` + `contacts` + `network` + `sensors`) before R2 paints; covering scopes and Network · Sensors toggles are load-bearing. Title (with B1) `SEVA B0+B1+R2+T1 GREEN` / `TUBE1 admitted · Pool x3`. Install proof GREEN; Keaton confirmed on Pixel.

**TUBE5 Mantra-rev tip on glass GREEN `20260717.140600`:** after admit + R2, `tube5_glass_tip.rye` lands Mantra rev1/rev2 and paints `TUBE5 tip rev2 · 0.2.0` (UI string — Mantra revision tip, not landed-edge *nib*). Install proof GREEN (binds `seva_root.txt`); Keaton confirmed the Mantra-rev line on Pixel.
