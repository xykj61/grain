...
- **`KYRI.md`** -- **Kyri**, the **standing voice**, molted from Riyo on `20260810` on Keaton's word; the voice and the tree's **Kyri notation** (`.kyri`) share one name, since the voice writes the record and the record is written in its notation.
- **`RIYO.md`** -- **Riyo**, the voice's chapter from `20260729.205200` until the molt to Kyri on `20260810` -- a fossil kept whole, never the living voice; one of Grain's own OS variants, paired with Reya. The first Riyo chapter rests in [`archive/`](archive/RIYO.md) as its elder record.
