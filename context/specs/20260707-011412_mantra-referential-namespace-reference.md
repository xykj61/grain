...
Correction pass `20260811.233044` -- sectionBeading function citations aligned to the landed snake_case code (`append_leaf_beaded` - `recall_beaded` - `hydrate_bead_store_from_catalog`), per this spec's own "prose disagrees -> code governs -> this document earns a correction" clause. Behavior unchanged; a citation fix, not a rewrite.

---

## section1 Scope and Conformance Terms
...
## section2 Names and Their Bounds
...
## section5 The Read Family

Each reader composes over section4 and **must** preserve its outcomes and refusals unchanged.
...
```
[whole_digest: 32 raw][whole_len: u16][bead_count: u16][bead_digests: count x 32 raw]
```
...
## section8 The Wire -- Sealed Datagrams and Sync Payloads
...
```
[kind:u8][peer_len:u8][peer][bolt_len:u8][bolt][revision:u32 LE][held_count:u8][held: count x 64 hex]
```
...
## section9 The Resin-Batch Frame (kind `0x03`)
...
count x [path field][tilak field][digest: 64 hex][length: u32 LE][has_payload: u8]
...
## section10 Chunking (kind `0x04`) and the Assembler
...
## section11 Witness and Parity Registry
...
## section12 Open Items (Non-Normative)
