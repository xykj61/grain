...
**First cut landed `20260811.131417`.** `tools/fixtures/labeling_module_scan.sh` scans given module living surfaces for the bare `lap N` identity pattern and reports a verdict; `tools/gen/season/cion_module_labeling_witness.rish` runs it over `mandate/` (README + all authored `.rye`) and proves the guard **bites** a planted negative (`tools/fixtures/labeling_module_negative.md`). `mandate/` was the **first swept module**, then **scribe + vault** (`20260811.133322`) — their capabilities now read as semantic label + stamp; the scanned set grows one module per CION round until the whole living corpus is covered. Larger and witness-coupled modules (`comlink/` — ~40 refs across the guest protocol files; `granary/` and `pond/`, whose `lap N` sits inside witness strings) are their own rounds. Session logs and dated specs stay out of the scan (testimony).
