...
| **Application Firewall** | **On, blocking all non-essential incoming connections** (confirmed `20260714.055300` via `socketfilterfw --getglobalstate`/`--getblockall`/`--getstealthmode`) -- **stealth mode also on** | **Closed**, and stronger than the base recommendation: block-all is active (State = 2), so the per-app "allow" list and the two "automatically allow signed software" toggles are inert leftovers, not live exceptions -- block-all overrides them, permitting nothing beyond basic services like DHCP/IPSec. Stealth mode means the Mac does not even acknowledge ping/probe attempts. This is the strictest posture available in System Settings |
