...
**Status:** Living front door — Season C opens (ALES0 wire · ALES1 stream · ALES2 timeline edits · ALES3 mix)
...
The next rung can **clock** the timeline to a real sample rate (samples gain a time base — seconds, not just indices, and two clips that start at different times can align), add a **fade** envelope (gain that ramps rather than steps), or open a small **track table** so more than two tracks mix at once. The audio-interface hardware stays a paused research round, taken only on Keaton's word.
